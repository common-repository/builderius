...
class PostACFFieldsDynamicDataHelpersProvider extends AbstractACFFieldsDynamicDataHelpersProvider
{
    private function getDefaultParams(array $field, $idx)
    {
        ...
        if (!isset($field['return_format']) || $field['return_format'] !== 'array') {
            $expression = '"wp.post.acf_field__' . $sanitizedName . '"';
        } else {
            $expression = '"wp.post.acf_field__' . $sanitizedName . '." ~ argument';
        }
        return [
            DynamicDataHelper::NAME_FIELD => 'post_acf_value_' . $sanitizedName,
            DynamicDataHelper::LABEL_FIELD => 'Post ACF "' . $field['label'] . '" field value',
            DynamicDataHelper::CATEGORY_FIELD => 'post',
            DynamicDataHelper::SORT_ORDER_FIELD => 300 + 10 * $idx,
            DynamicDataHelper::GRAPHQL_PATH_FIELD => [
                '"query.post.__aliasFor"' => '"queried_post"',
                '"query.post.acf_field__' . $sanitizedName. '.__aliasFor"' => '"acf_value"',
                '"query.post.acf_field__' . $sanitizedName. '.__args.name"' => '"' . $field['name'] . '"',
            ],
            ...
            foreach ($acf_field_groups as $acf_field_group) {
                ...
                foreach ($acf_field_group['location'] as $group_locations) {
                    foreach ($group_locations as $rule) {
                        if (
                            strpos($rule['param'], 'post') !== false ||
                            (strpos($rule['param'], 'page') !== false && $rule['param'] !== 'options_page')
                        ) {
                            $applicable = true;
                            break;
                        }
    ...
    private function addScalarFieldHelper(array $field, $idx)
    {
        ...
        $sanitizedName = preg_replace('/[^\\w]+/', '_', $field['name'], -1);
        $params[DynamicDataHelper::EXPRESSION_FIELD] = '"wp.post.acf_field__' . $sanitizedName . '"';
    ...
    private function addGoogleMapFieldHelper(array $field, $idx)
    {
        $params = $this->getDefaultParams($field, $idx);
        $sanitizedName = preg_replace('/[^\\w]+/', '_', $field['name'], -1);
        $params[DynamicDataHelper::EXPRESSION_FIELD] = '"wp.post.acf_field__' . $sanitizedName . '." ~ argument';
    ...
    private function addFlexibleContentFieldHelper(array $field, $idx)
    {
        ...
        $params[DynamicDataHelper::GRAPHQL_PATH_FIELD] = [
            '"query.post.__aliasFor"' => '"queried_post"',
            '"query.post.acf_field__' . $sanitizedName. '_private.__aliasFor"' => '"acf_value"',
            '"query.post.acf_field__' . $sanitizedName. '_private.__args.name"' => '"' . $field['name'] . '"',
            '"query.post.acf_field__' . $sanitizedName. '_private.__directives.private"' => true,
            '"query.post.acf_field__' . $sanitizedName. '.__aliasFor"' => '"expression_result"',
            '"query.post.acf_field__' . $sanitizedName. '.__args.expression"' => '"{{{foreach(group_by(acf_field__' . $sanitizedName. '_private, \'acf_fc_layout\'), (value) -> {value[0]}) }}}"',
        ];
        $params[DynamicDataHelper::EXPRESSION_FIELD] = '"wp.post.acf_field__' . $sanitizedName . '." ~ argument';
    ...
    private function addPostObjectFieldHelper(array $field, $idx)
    {
        ...
        $params[DynamicDataHelper::GRAPHQL_PATH_FIELD] = [
            '"query.post.__aliasFor"' => '"queried_post"',
            '"query.post.acf_field__' . $sanitizedName. '.__aliasFor"' => '"acf_post_object_value"',
            '"query.post.acf_field__' . $sanitizedName. '.__args.name"' => '"' . $field['name'] . '"',
            '"query.post.acf_field__' . $sanitizedName. '." ~ (isset(config[argument], "parent_alias") ? config[argument].parent_alias : config[argument].alias) ~ ".__aliasFor"' => 'isset(config[argument], "parent_field") ? config[argument].parent_field : config[argument].field',
            '"query.post.acf_field__' . $sanitizedName. '." ~ (isset(config[argument], "parent_alias") ? config[argument].parent_alias ~ "." : "") ~ config[argument].alias ~ ".__aliasFor"' => 'config[argument].field',
        ];
        $params[DynamicDataHelper::EXPRESSION_FIELD] = '"wp.post.acf_field__' . $sanitizedName . '." ~ (isset(config[argument], "parent_alias") ? config[argument].parent_alias ~ "." : "") ~ config[argument].alias';
    ...
    private function addUserFieldHelper(array $field, $idx)
    {
        ...
        $params[DynamicDataHelper::GRAPHQL_PATH_FIELD] = [
            '"query.post.__aliasFor"' => '"queried_post"',
            '"query.post.acf_field__' . $sanitizedName. '.__aliasFor"' => '"acf_user_value"',
            '"query.post.acf_field__' . $sanitizedName. '.__args.name"' => '"' . $field['name'] . '"',
            '"query.post.acf_field__' . $sanitizedName. '." ~ config[argument].alias ~ ".__aliasFor"' => 'config[argument].field',
        ];
        $params[DynamicDataHelper::EXPRESSION_FIELD] = '"wp.post.acf_field__' . $sanitizedName . '." ~ config[argument].alias';
