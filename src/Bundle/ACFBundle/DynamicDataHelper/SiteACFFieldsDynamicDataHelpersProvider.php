...
class SiteACFFieldsDynamicDataHelpersProvider extends AbstractACFFieldsDynamicDataHelpersProvider
{
    private function getDefaultParams(array $field, $idx)
    {
        ...
        if (!isset($field['return_format']) || $field['return_format'] !== 'array') {
            $expression = '"wp.acf_options_field__' . $sanitizedName . '"';
        } else {
            $expression = '"wp.acf_options_field__' . $sanitizedName . '." ~ argument';
        }
        return [
            DynamicDataHelper::NAME_FIELD => 'site_acf_value_' . $sanitizedName,
            DynamicDataHelper::LABEL_FIELD => 'ACF Options page "' . $field['label'] . '" field value',
            DynamicDataHelper::CATEGORY_FIELD => 'site',
            DynamicDataHelper::SORT_ORDER_FIELD => 300 + 10 * $idx,
            DynamicDataHelper::GRAPHQL_PATH_FIELD => [
                '"query.acf_options_field__' . $sanitizedName. '.__aliasFor"' => '"acf_option_value"',
                '"query.acf_options_field__' . $sanitizedName. '.__args.name"' => '"' . $field['name'] . '"',
            ],
    ...
    private function addScalarFieldHelper(array $field, $idx)
    {
        ...
        $sanitizedName = preg_replace('/[^\\w]+/', '_', $field['name'], -1);
        $params[DynamicDataHelper::EXPRESSION_FIELD] = '"wp.acf_options_field__' . $sanitizedName . '"';
    ...
    private function addGoogleMapFieldHelper(array $field, $idx)
    {
        $params = $this->getDefaultParams($field, $idx);
        $sanitizedName = preg_replace('/[^\\w]+/', '_', $field['name'], -1);
        $params[DynamicDataHelper::EXPRESSION_FIELD] = '"wp.acf_options_field__' . $sanitizedName . '." ~ argument';
    ...
    private function addFlexibleContentFieldHelper(array $field, $idx)
    {
        ...
        $params[DynamicDataHelper::GRAPHQL_PATH_FIELD] = [
            '"query.acf_options_field__' . $sanitizedName. '_private.__aliasFor"' => '"acf_option_value"',
            '"query.acf_options_field__' . $sanitizedName. '_private.__args.name"' => '"' . $field['name'] . '"',
            '"query.acf_options_field__' . $sanitizedName. '_private.__directives"' => '{"private": true}',
            '"query.acf_options_field__' . $sanitizedName. '.__aliasFor"' => '"expression_result"',
            '"query.acf_options_field__' . $sanitizedName. '.__args.expression"' => '"{{{foreach(group_by(acf_field__' . $sanitizedName. '_private, \'acf_fc_layout\'), (value) -> {value[0]}) }}}"',
        ];
        $params[DynamicDataHelper::EXPRESSION_FIELD] = '"wp.acf_options_field__' . $sanitizedName . '." ~ argument';
    ...
    private function addPostObjectFieldHelper(array $field, $idx)
    {
        ...
        $sanitizedName = preg_replace('/[^\\w]+/', '_', $field['name'], -1);
        $params[DynamicDataHelper::GRAPHQL_PATH_FIELD] = [
            '"query.acf_options_field__' . $sanitizedName. '.__aliasFor"' => '"acf_option_post_object_value"',
            '"query.acf_options_field__' . $sanitizedName. '.__args.name"' => '"' . $field['name'] . '"',
            '"query.acf_options_field__' . $sanitizedName. '." ~ (isset(config[argument], "parent_alias") ? config[argument].parent_alias : config[argument].alias) ~ ".__aliasFor"' => 'isset(config[argument], "parent_field") ? config[argument].parent_field : config[argument].field',
            '"query.acf_options_field__' . $sanitizedName. '." ~ (isset(config[argument], "parent_alias") ? config[argument].parent_alias ~ "." : "") ~ config[argument].alias ~ ".__aliasFor"' => 'config[argument].field',
        ];
        $params[DynamicDataHelper::EXPRESSION_FIELD] = '"wp.acf_options_field__' . $sanitizedName . '." ~ (isset(config[argument], "parent_alias") ? config[argument].parent_alias ~ "." : "") ~ config[argument].alias';
    ...
    private function addUserFieldHelper(array $field, $idx)
    {
        ...
        $sanitizedName = preg_replace('/[^\\w]+/', '_', $field['name'], -1);
        $params[DynamicDataHelper::GRAPHQL_PATH_FIELD] = [
            '"query.acf_options_field__' . $sanitizedName. '.__aliasFor"' => '"acf_option_user_value"',
            '"query.acf_options_field__' . $sanitizedName. '.__args.name"' => '"' . $field['name'] . '"',
            '"query.acf_options_field__' . $sanitizedName. '." ~ config[argument].alias ~ ".__aliasFor"' => 'config[argument].field',
        ];
        $params[DynamicDataHelper::EXPRESSION_FIELD] = '"wp.acf_options_field__' . $sanitizedName . '." ~ config[argument].alias';
