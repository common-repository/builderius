...
class CompositeBuilderiusModulesProvider implements BuilderiusModulesProviderInterface
{
    /**
    ...
    {
        $modules = $this->cache->get(sprintf('builderius_%s_%s_modules_%s_%s', $templateType, $technology, $withSettings, json_encode($configVersions)));
        if (false === $modules) {
            ...
            $this->cache->set(sprintf('builderius_%s_%s_modules_%s_%s', $templateType, $technology, $withSettings, json_encode($configVersions)), $modules);
            $this->modules = [];
        }
