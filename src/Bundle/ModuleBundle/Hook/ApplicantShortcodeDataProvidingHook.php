...
class ApplicantShortcodeDataProvidingHook extends AbstractApplicantDataAction
{
    /**
    ...
    public function getFunction()
    {
        ...
        if (isset( $_POST['builderius-applicant-shortcode-data']) && isset($_POST['uniqid']) && user_can($user, 'builderius-development')) {
            ...
            $shortcodeContent = $this->runtimeCache->get(sprintf('applicant_shortcode_content_%s', $_POST['uniqid']));
            $applicantData['content'] = $shortcodeContent;

            /** @var CacheItem $dataCacheItem */
            $dataCacheItem = $this->persistentCache->getItem(sprintf('applicant_shortcode_data_%s', $_POST['uniqid']));
