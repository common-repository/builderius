...
class BuilderiusApplicantGbBlocksDataEndpoint implements RestApiEndpointInterface
{
    const NAMESPACE = 'wp/v2';
    const REST_BASE = 'builderius-applicant-gbblocks-data';
     ...
     */
    public function getGbBlockData(\WP_REST_Request $request)
    {
        ...
        foreach ($data['items'] as $block) {
            $uniqid = uniqid();
            /** @var CacheItem $blockCacheItem */
            $blockCacheItem = $this->persistentCache->getItem(sprintf('applicant_gbblock_%s', $uniqid));
            $blockCacheItem->set($block);
            $this->persistentCache->save($blockCacheItem);
            ...
            $args = [
                'method' => 'POST',
                'body' => [
                    'builderius-applicant-gbblock-data' => true,
                    'uniqid' => $uniqid,
                ],
                'timeout' => -1,
