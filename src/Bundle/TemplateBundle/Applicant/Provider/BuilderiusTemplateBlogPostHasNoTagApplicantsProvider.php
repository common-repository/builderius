...
class BuilderiusTemplateBlogPostHasNoTagApplicantsProvider extends AbstractBuilderiusTemplateSingularApplicantsProvider
{
    /**
     * @var \WP_Query
     */
    ...
    public function getChangeSetApplicants(
        ...
        if (!$object instanceof \WP_Post || $object->post_type !== 'post' || has_tag($argument, $object)) {
            return [];
        }
    ...
    {
        if (strpos($rule, 'singular.single.blog_post.has_tag') !== false && is_integer($argument) && $operator === '!=') {
            return true;
        }
