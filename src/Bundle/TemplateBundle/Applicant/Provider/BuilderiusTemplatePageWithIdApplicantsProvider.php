...
class BuilderiusTemplatePageWithIdApplicantsProvider extends AbstractBuilderiusTemplateSingularApplicantsProvider
{
    /**
     * @var \WP_Query
     */
            ...
            if ($frontpageId && $showOnFront === 'page' && (int)$object->ID == (int)$frontpageId) {
                return [
                    sprintf('singular.single.%s.%s', $object->post_type, $object->ID) =>
                        $this->convertPostToApplicant($object, sprintf('%s: %s', __('Front page'), $object->post_title), $withData, 'special pages')
                ];
            } else {
                return [
                    sprintf('singular.single.%s.%s', $object->post_type, $object->ID) =>
