...
class Cleaner
{
    ...
    public function __construct(\Composer\IO\IOInterface $io, \Composer\Util\Filesystem $fileSystem)
    {
        $this->io = $io;
    ...
    public function clean($vendorDir, array $ignorePaths = []) : void
    {
        foreach (new \FilesystemIterator($vendorDir) as $packageVendor) {
            ...
            foreach (new \FilesystemIterator((string) $packageVendor) as $packageName) {
                ...
                if ($ignore === \true) {
                    $this->io->write("Composer cleaner: Skipped package {$name}", \true, \Composer\IO\IOInterface::VERBOSE);
                } else {
                    $this->io->write("Composer cleaner: Package {$name}", \true, \Composer\IO\IOInterface::VERBOSE);
    ...
    private function processPackage(string $packageDir, array $ignoreFiles) : void
    {
        ...
        foreach ($this->getExcludes($data) as $exclude) {
            ...
            if ($dir && \strpos($dir, '..') === \false && !self::matchMask($dir, $ignoreFiles)) {
                $path = $packageDir . '/' . $dir;
                $this->io->write("Composer cleaner: Removing {$path}", \true, \Composer\IO\IOInterface::VERBOSE);
    ...
    public function loadComposerJson(string $dir) : ?\stdClass
    {
        ...
        if (!\is_file($file)) {
            $this->io->writeError("Composer cleaner: File {$file} not found.", \true, \Composer\IO\IOInterface::VERBOSE);
            return null;
        }
