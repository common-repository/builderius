...
class Subject
{
    ...
    public function hasInfiniteLoop(array $subjects = [], $level = 1)
    {
        if ($level > $this->useNestingLimit) {
            throw new \Builderius\enshrined\svgSanitize\Exceptions\NestingException('Nesting level too high, aborting', 1570713498, null, $this->getElement());
        }
     ...
     */
    public function addUse(\Builderius\enshrined\svgSanitize\ElementReference\Subject $subject)
    {
        if ($subject === $this) {
        ...
        $this->useCollection[$identifier] = new \Builderius\enshrined\svgSanitize\ElementReference\Usage($subject);
    }
    /**
     * @param Subject $subject
     */
    public function addUsedIn(\Builderius\enshrined\svgSanitize\ElementReference\Subject $subject)
    {
        if ($subject === $this) {
        ...
        $this->usedInCollection[$identifier] = new \Builderius\enshrined\svgSanitize\ElementReference\Usage($subject);
    }
    /**
    ...
    {
        $elements = \array_map(function (\Builderius\enshrined\svgSanitize\ElementReference\Usage $usage) {
