 ...
 * @internal
 */
final class SafeAnalysisNodeVisitor implements \Builderius\Twig\NodeVisitor\NodeVisitorInterface
{
    private $data = [];
    private $safeVars = [];
    ...
    public function getSafe(\Builderius\Twig\Node\Node $node)
    {
    ...
    private function setSafe(\Builderius\Twig\Node\Node $node, array $safe) : void
    {
        $hash = \spl_object_hash($node);
        if (isset($this->data[$hash])) {
            foreach ($this->data[$hash] as &$bucket) {
    ...
    public function leaveNode(\Builderius\Twig\Node\Node $node, \Builderius\Twig\Environment $env) : ?\Builderius\Twig\Node\Node
    {
        if ($node instanceof \Builderius\Twig\Node\Expression\ConstantExpression) {
            ...
            $this->setSafe($node, ['all']);
        } elseif ($node instanceof \Builderius\Twig\Node\Expression\ParentExpression) {
            // parent block is safe by definition
            $this->setSafe($node, ['all']);
        } elseif ($node instanceof \Builderius\Twig\Node\Expression\ConditionalExpression) {
        ...
        } elseif ($node instanceof \Builderius\Twig\Node\Expression\FunctionExpression) {
        ...
        } elseif ($node instanceof \Builderius\Twig\Node\Expression\GetAttrExpression && $node->getNode('node') instanceof \Builderius\Twig\Node\Expression\NameExpression) {
