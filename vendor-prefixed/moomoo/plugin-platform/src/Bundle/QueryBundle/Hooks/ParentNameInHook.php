...
class ParentNameInHook extends \Builderius\MooMoo\Platform\Bundle\HookBundle\Model\AbstractFilter
{
    ...
    public function getFunction()
    {
        ...
        if (isset($query->query[self::QUERY_ARGUMENT]) && \is_array($query->query[self::QUERY_ARGUMENT])) {
            ...
            if (!empty($names)) {
                $where = \sprintf("%s AND %s.post_name IN ('%s')", $where, \Builderius\MooMoo\Platform\Bundle\QueryBundle\Hooks\PostParentJoinHook::PARENT_TABLE, \implode("','", $names));
            }
        }
        return $where;
    }
