 ...
 */
abstract class AbstractNotPartitionedDatabase extends \Builderius\Sokil\IsoCodes\AbstractDatabase
{
    /**
    ...
    private function getIndex(string $indexedFieldName) : array
    {
                        ...
                        if (\is_array($indexDefinition)) {
                            ...
                            foreach ($indexDefinition as $indexDefinitionPart) {
                                if (\is_array($indexDefinitionPart)) {
                                    ...
                                    $indexDefinitionPartValue = \substr($entryArray[$indexDefinitionPart[0]], 0, $indexDefinitionPart[1]);
                                } else {
        ...
        if (!isset($this->index[$indexedFieldName])) {
            throw new \InvalidArgumentException(\sprintf('Unknown index "%s" in database "%s"', $indexedFieldName, \get_class()));
        }
