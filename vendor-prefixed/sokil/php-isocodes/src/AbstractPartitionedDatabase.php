 ...
 */
abstract class AbstractPartitionedDatabase extends \Builderius\Sokil\IsoCodes\AbstractDatabase
{
    /**
    ...
    {
        $pathToPartitionFile = \sprintf('%s/iso_%s/%s.json', $this->getDatabasesPath(), $this->getISONumber(), $fileName);
        if (!\file_exists($pathToPartitionFile)) {
            return [];
        }
        return \json_decode(\file_get_contents($pathToPartitionFile), \true);
    }
}
