...
class Tokenizer
{
    ...
    // Valid token types
    private static $tagTypes = array(self::T_SECTION => \true, self::T_INVERTED => \true, self::T_END_SECTION => \true, self::T_COMMENT => \true, self::T_PARTIAL => \true, self::T_PARENT => \true, self::T_DELIM_CHANGE => \true, self::T_ESCAPED => \true, self::T_UNESCAPED => \true, self::T_UNESCAPED_2 => \true, self::T_PRAGMA => \true, self::T_BLOCK_VAR => \true);
    // Token properties
    const TYPE = 'type';
    const NAME = 'name';
    const OTAG = 'otag';
    ...
    public function scan($text, $delimiters = '')
    {
        ...
        $this->reset();
        if (\is_string($delimiters) && ($delimiters = \trim($delimiters))) {
            ...
            switch ($this->state) {
                ...
                default:
                    ...
                    if ($char === $this->ctagChar && \substr($text, $i, $this->ctagLen) === $this->ctag) {
                        $token = array(self::TYPE => $this->tagType, self::NAME => \trim($this->buffer), self::OTAG => $this->otag, self::CTAG => $this->ctag, self::LINE => $this->line, self::INDEX => $this->tagType === self::T_END_SECTION ? $this->seenTag - $this->otagLen : $i + $this->ctagLen);
    ...
    private function flushBuffer()
    {
        if (\strlen($this->buffer) > 0) {
            $this->tokens[] = array(self::TYPE => self::T_TEXT, self::LINE => $this->line, self::VALUE => $this->buffer);
            $this->buffer = '';
        }
    }
    /**
    ...
    private function changeDelimiters($text, $index)
    {
        ...
        $token = array(self::TYPE => self::T_DELIM_CHANGE, self::LINE => $this->line);
        try {
            $this->setDelimiters(\trim(\substr($text, $startIndex, $closeIndex - $startIndex)));
        } catch (\Builderius\Mustache\Exception\InvalidArgumentException $e) {
            throw new \Builderius\Mustache\Exception\SyntaxException($e->getMessage(), $token);
        }
        $this->tokens[] = $token;
    ...
    private function setDelimiters($delimiters)
    {
        if (!\preg_match('/^\\s*(\\S+)\\s+(\\S+)\\s*$/', $delimiters, $matches)) {
            throw new \Builderius\Mustache\Exception\InvalidArgumentException(\sprintf('Invalid delimiters: %s', $delimiters));
        }
    ...
    private function addPragma($text, $index)
    {
        ...
        \array_unshift($this->tokens, array(self::TYPE => self::T_PRAGMA, self::NAME => $pragma, self::LINE => 0));
        return $end + $this->ctagLen - 1;
    }
    private function throwUnclosedTagException()
    {
        $name = \trim($this->buffer);
        if ($name !== '') {
            $msg = \sprintf('Unclosed tag: %s on line %d', $name, $this->line);
        } else {
            $msg = \sprintf('Unclosed tag on line %d', $this->line);
        }
        throw new \Builderius\Mustache\Exception\SyntaxException($msg, array(self::TYPE => $this->tagType, self::NAME => $name, self::OTAG => $this->otag, self::CTAG => $this->ctag, self::LINE => $this->line, self::INDEX => $this->seenTag - $this->otagLen));
    }
}
