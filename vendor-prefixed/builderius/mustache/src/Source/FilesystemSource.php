...
namespace Builderius\Mustache\Source;

use Builderius\Mustache\Exception\RuntimeException;
use Builderius\Mustache\Source;
/**
 ...
 */
class FilesystemSource implements \Builderius\Mustache\Source
{
    private $fileName;
    private $statProps;
    private $stat;
    ...
    {
        $chunks = array('fileName' => $this->fileName);
        if (!empty($this->statProps)) {
            if (!isset($this->stat)) {
                $this->stat = @\stat($this->fileName);
            }
            if ($this->stat === \false) {
                throw new \Builderius\Mustache\Exception\RuntimeException(\sprintf('Failed to read source file "%s".', $this->fileName));
            }
