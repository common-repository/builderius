...
namespace Builderius\Mustache\Logger;

use Builderius\Mustache\Exception\InvalidArgumentException;
use Builderius\Mustache\Exception\LogicException;
 ...
 */
class StreamLogger extends \Builderius\Mustache\Logger\AbstractLogger
{
     ...
     */
    public function __construct($stream, $level = \Builderius\Mustache\Logger::ERROR)
    {
        $this->setLevel($level);
    ...
    public function setLevel($level)
    {
        if (!\array_key_exists($level, self::$levels)) {
            throw new \Builderius\Mustache\Exception\InvalidArgumentException(\sprintf('Unexpected logging level: %s', $level));
        }
        $this->level = $level;
    }
    /**
    ...
    protected function writeLog($level, $message, array $context = array())
    {
        if (!\is_resource($this->stream)) {
            if (!isset($this->url)) {
                throw new \Builderius\Mustache\Exception\LogicException('Missing stream url, the stream can not be opened. This may be caused by a premature call to close().');
            }
            $this->stream = \fopen($this->url, 'a');
            if (!\is_resource($this->stream)) {
                // @codeCoverageIgnoreStart
                throw new \Builderius\Mustache\Exception\RuntimeException(\sprintf('The stream or file "%s" could not be opened.', $this->url));
