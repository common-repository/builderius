 ...
 *         'mustache.loader' => new Loader_InlineLoader(__FILE__, __COMPILER_HALT_OFFSET__)
 *     ));
 *
 ...
 *     Hello, {{ name }}!
 */
class InlineLoader implements \Builderius\Mustache\Loader
{
    protected $fileName;
    protected $offset;
    protected $templates;
    ...
    {
        if (!\is_file($fileName)) {
            throw new \Builderius\Mustache\Exception\InvalidArgumentException('InlineLoader expects a valid filename.');
        }
        if (!\is_int($offset) || $offset < 0) {
            throw new \Builderius\Mustache\Exception\InvalidArgumentException('InlineLoader expects a valid file offset.');
        }
