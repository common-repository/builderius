 ...
 */
class EventManager implements \Builderius\Zend\EventManager\EventManagerInterface
{
    /**
     ...
     */
    public function __construct(\Builderius\Zend\EventManager\SharedEventManagerInterface $sharedEventManager = null, array $identifiers = [])
    {
        if ($sharedEventManager) {
            $this->sharedManager = $sharedEventManager;
            $this->setIdentifiers($identifiers);
        }
        $this->eventPrototype = new \Builderius\Zend\EventManager\Event();
    }
    /**
     * @inheritDoc
     */
    public function setEventPrototype(\Builderius\Zend\EventManager\EventInterface $prototype)
    {
     ...
     * @inheritDoc
     */
    public function triggerEvent(\Builderius\Zend\EventManager\EventInterface $event)
    {
        return $this->triggerListeners($event);
    }
    /**
     * @inheritDoc
     */
    public function triggerEventUntil(callable $callback, \Builderius\Zend\EventManager\EventInterface $event)
    {
        return $this->triggerListeners($event, $callback);
    }
    /**
    ...
    public function attach($eventName, callable $listener, $priority = 1)
    {
        if (!\is_string($eventName)) {
            throw new \Builderius\Zend\EventManager\Exception\InvalidArgumentException(\sprintf('%s expects a string for the event; received %s', __METHOD__, \is_object($eventName) ? \get_class($eventName) : \gettype($eventName)));
        }
     ...
     */
    protected function triggerListeners(\Builderius\Zend\EventManager\EventInterface $event, callable $callback = null)
    {
        $name = $event->getName();
        if (empty($name)) {
            throw new \Builderius\Zend\EventManager\Exception\RuntimeException('Event is missing a name; cannot trigger!');
        }
        ...
        // Execute listeners
        $responses = new \Builderius\Zend\EventManager\ResponseCollection();
