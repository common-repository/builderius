...
class LazyListener
{
     ...
     * @param array $env
     */
    public function __construct(array $definition, \Builderius\Interop\Container\ContainerInterface $container, array $env = [])
    {
        if (!isset($definition['listener']) || !\is_string($definition['listener']) || empty($definition['listener'])) {
            throw new \Builderius\Zend\EventManager\Exception\InvalidArgumentException('Lazy listener definition is missing a valid "listener" member; cannot create LazyListener');
        }
        if (!isset($definition['method']) || !\is_string($definition['method']) || empty($definition['method'])) {
            throw new \Builderius\Zend\EventManager\Exception\InvalidArgumentException('Lazy listener definition is missing a valid "method" member; cannot create LazyListener');
        }
     ...
     * @return callable
     */
    public function __invoke(\Builderius\Zend\EventManager\EventInterface $event)
    {
