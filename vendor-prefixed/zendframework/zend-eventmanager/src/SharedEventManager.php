 ...
 */
class SharedEventManager implements \Builderius\Zend\EventManager\SharedEventManagerInterface
{
    /**
    ...
    public function attach($identifier, $event, callable $listener, $priority = 1)
    {
        if (!\is_string($identifier) || empty($identifier)) {
            throw new \Builderius\Zend\EventManager\Exception\InvalidArgumentException(\sprintf('Invalid identifier provided; must be a string; received "%s"', \is_object($identifier) ? \get_class($identifier) : \gettype($identifier)));
        }
        if (!\is_string($event) || empty($event)) {
            throw new \Builderius\Zend\EventManager\Exception\InvalidArgumentException(\sprintf('Invalid event provided; must be a non-empty string; received "%s"', \is_object($event) ? \get_class($event) : \gettype($event)));
        }
    ...
    public function detach(callable $listener, $identifier = null, $eventName = null, $force = \false)
    {
        ...
        if (!\is_string($eventName) || empty($eventName)) {
            throw new \Builderius\Zend\EventManager\Exception\InvalidArgumentException(\sprintf('Invalid event name provided; must be a string, received %s', \is_object($eventName) ? \get_class($eventName) : \gettype($eventName)));
        }
    ...
    public function getListeners(array $identifiers, $eventName)
    {
        if ('*' === $eventName || !\is_string($eventName) || empty($eventName)) {
            throw new \Builderius\Zend\EventManager\Exception\InvalidArgumentException(\sprintf('Event name passed to %s must be a non-empty, non-wildcard string', __METHOD__));
        }
        $returnListeners = [];
        foreach ($identifiers as $identifier) {
            if ('*' === $identifier || !\is_string($identifier) || empty($identifier)) {
                throw new \Builderius\Zend\EventManager\Exception\InvalidArgumentException(\sprintf('Identifier names passed to %s must be non-empty, non-wildcard strings', __METHOD__));
            }
            if (isset($this->identifiers[$identifier])) {
