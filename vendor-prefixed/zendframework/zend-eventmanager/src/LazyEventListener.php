 ...
 */
class LazyEventListener extends \Builderius\Zend\EventManager\LazyListener
{
    /**
    ...
    public function __construct(array $definition, \Builderius\Interop\Container\ContainerInterface $container, array $env = [])
    {
        ...
        if (!isset($definition['event']) || !\is_string($definition['event']) || empty($definition['event'])) {
            throw new \Builderius\Zend\EventManager\Exception\InvalidArgumentException('Lazy listener definition is missing a valid "event" member; cannot create LazyListener');
        }
