 ...
 */
namespace Builderius\Zend\Code\Generic\Prototype;

use Builderius\Zend\Code\Reflection\Exception;
...
class PrototypeClassFactory
{
     ...
     */
    public function __construct($prototypes = [], \Builderius\Zend\Code\Generic\Prototype\PrototypeGenericInterface $genericPrototype = null)
    {
     ...
     */
    public function addPrototype(\Builderius\Zend\Code\Generic\Prototype\PrototypeInterface $prototype)
    {
        $prototypeName = $this->normalizeName($prototype->getName());
        if (isset($this->prototypes[$prototypeName])) {
            throw new \Builderius\Zend\Code\Reflection\Exception\InvalidArgumentException('A prototype with this name already exists in this manager');
        }
     ...
     * @throws Exception\InvalidArgumentException
     */
    public function setGenericPrototype(\Builderius\Zend\Code\Generic\Prototype\PrototypeGenericInterface $prototype)
    {
        if (isset($this->genericPrototype)) {
            throw new \Builderius\Zend\Code\Reflection\Exception\InvalidArgumentException('A default prototype is already set');
        }
    ...
    public function getClonedPrototype($prototypeName)
    {
        ...
        if (!$this->hasPrototype($prototypeName) && !isset($this->genericPrototype)) {
            throw new \Builderius\Zend\Code\Reflection\Exception\RuntimeException('This tag name is not supported by this tag manager');
        }
