...
namespace Builderius\Zend\Code;
...
class DeclareStatement
{
    ...
    public static function fromArray(array $config) : self
    {
        ...
        if (!isset(self::ALLOWED[$directive])) {
            throw new \Builderius\Zend\Code\Exception\InvalidArgumentException(\sprintf('Declare directive must be one of: %s.', \implode(', ', \array_keys(self::ALLOWED))));
        }
        if (\gettype($value) !== self::ALLOWED[$directive]) {
            throw new \Builderius\Zend\Code\Exception\InvalidArgumentException(\sprintf('Declare value invalid. Expected %s, got %s.', self::ALLOWED[$directive], \gettype($value)));
        }
