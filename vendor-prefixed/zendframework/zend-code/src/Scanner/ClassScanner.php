...
use ReflectionClass;
use Builderius\Zend\Code\Annotation;
use Builderius\Zend\Code\Exception;
use Builderius\Zend\Code\NameInformation;
...
class ClassScanner implements \Builderius\Zend\Code\Scanner\ScannerInterface
{
    /**
     * @var bool
     */
     ...
     * @return ClassScanner
     */
    public function __construct(array $classTokens, \Builderius\Zend\Code\NameInformation $nameInformation = null)
    {
    ...
    public function getAnnotations(\Builderius\Zend\Code\Annotation\AnnotationManager $annotationManager)
    {
        ...
        return new \Builderius\Zend\Code\Scanner\AnnotationScanner($annotationManager, $docComment, $this->nameInformation);
    }
    /**
    ...
    public function getDocBlock()
    {
        ...
        return new \Builderius\Zend\Code\Scanner\DocBlockScanner($docComment);
    }
    /**
    ...
    {
        if (\true === $namesOnly) {
            \trigger_error('Use method getConstantNames() instead', \E_USER_DEPRECATED);
    ...
    public function getConstant($constantNameOrInfoIndex)
    {
        ...
        if (\is_int($constantNameOrInfoIndex)) {
            ...
            if ($info['type'] != 'constant') {
                throw new \Builderius\Zend\Code\Exception\InvalidArgumentException('Index of info offset is not about a constant');
            }
            ...
            throw new \Builderius\Zend\Code\Exception\InvalidArgumentException('Invalid constant name of info index type.  Must be of type int or string');
        }
        if (!isset($info)) {
            return \false;
        }
        $p = new \Builderius\Zend\Code\Scanner\ConstantScanner(\array_slice($this->tokens, $info['tokenStart'], $info['tokenEnd'] - $info['tokenStart'] + 1), $this->nameInformation);
    ...
    public function getProperty($propertyNameOrInfoIndex)
    {
        ...
        if (\is_int($propertyNameOrInfoIndex)) {
            ...
            if ($info['type'] != 'property') {
                throw new \Builderius\Zend\Code\Exception\InvalidArgumentException('Index of info offset is not about a property');
            }
            ...
            throw new \Builderius\Zend\Code\Exception\InvalidArgumentException('Invalid property name of info index type.  Must be of type int or string');
        }
        if (!isset($info)) {
            return \false;
        }
        $p = new \Builderius\Zend\Code\Scanner\PropertyScanner(\array_slice($this->tokens, $info['tokenStart'], $info['tokenEnd'] - $info['tokenStart'] + 1), $this->nameInformation);
        $p->setClass($this->name);
        $p->setScannerClass($this);
        return $p;
    }
    ...
    public function getTraits()
    {
        ...
        foreach ($traitNames as $traitName) {
            ...
            if (!$r->isTrait()) {
                throw new \Builderius\Zend\Code\Exception\RuntimeException(\sprintf('Non-trait class detected as a trait: %s', $traitName));
            }
            $fileName = $r->getFileName();
            $file = new \Builderius\Zend\Code\Scanner\FileScanner($fileName);
        ...
        foreach ($this->infos as $info) {
            ...
            if (\is_array($info['use_statements'])) {
                foreach ($info['use_statements'] as $trait) {
                    $traitName = $trait;
                    if ($this->nameInformation instanceof \Builderius\Zend\Code\NameInformation) {
    ...
    public function getMethods()
    {
        ...
        foreach ($this->infos as $info) {
                    ...
                    foreach ($tempMethods as $tempMethod) {
                        ...
                        if (\false !== $methodAlias) {
                            ...
                            if (\array_key_exists($tempMethod->getName(), $traitMethods)) {
                                throw new \Builderius\Zend\Code\Exception\RuntimeException(\sprintf('Trait method %s has not been applied because there are' . ' collisions with other trait methods see: (insteadof OR as)', $tempMethod->getName()));
                            }
            ...
            $m = new \Builderius\Zend\Code\Scanner\MethodScanner(\array_slice($this->tokens, $info['tokenStart'], $info['tokenEnd'] - $info['tokenStart'] + 1), $this->nameInformation);
    ...
    public function getMethod($methodNameOrInfoIndex)
    {
        ...
        if (\is_int($methodNameOrInfoIndex)) {
            ...
            if ($info['type'] != 'method') {
                throw new \Builderius\Zend\Code\Exception\InvalidArgumentException('Index of info offset is not about a method');
            }
    ...
    protected function scan()
    {
        ...
        if (!$this->tokens) {
            throw new \Builderius\Zend\Code\Exception\RuntimeException('No tokens were provided');
        }
        /**
         * Variables & Setup
         */
