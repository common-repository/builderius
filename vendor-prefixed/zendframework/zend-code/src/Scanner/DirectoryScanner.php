...
class DirectoryScanner implements \Builderius\Zend\Code\Scanner\ScannerInterface
{
    /**
     * @var bool
     */
    ...
    {
        if ($directory instanceof \Builderius\Zend\Code\Scanner\DirectoryScanner) {
            $this->directories[] = $directory;
        } elseif (\is_string($directory)) {
            $realDir = \realpath($directory);
            if (!$realDir || !\is_dir($realDir)) {
                throw new \Builderius\Zend\Code\Exception\InvalidArgumentException(\sprintf('Directory "%s" does not exist', $realDir));
            }
            $this->directories[] = $realDir;
        } else {
            throw new \Builderius\Zend\Code\Exception\InvalidArgumentException('The argument provided was neither a DirectoryScanner or directory path');
        }
    }
    /**
     ...
     * @return void
     */
    public function addDirectoryScanner(\Builderius\Zend\Code\Scanner\DirectoryScanner $directoryScanner)
    {
     ...
     * @return void
     */
    public function addFileScanner(\Builderius\Zend\Code\Scanner\FileScanner $fileScanner)
    {
            ...
            if ($directory instanceof \Builderius\Zend\Code\Scanner\DirectoryScanner) {
                ...
                foreach (new \RecursiveIteratorIterator($rdi) as $item) {
                    if ($item->isFile() && \pathinfo($item->getRealPath(), \PATHINFO_EXTENSION) == 'php') {
                        $this->fileScanners[] = new \Builderius\Zend\Code\Scanner\FileScanner($item->getRealPath());
                    }
                }
            }
        }
    ...
    public function getClasses($returnDerivedScannerClass = \false)
    {
        ...
        foreach ($this->classToFileScanner as $className => $fsIndex) {
            ...
            if ($returnDerivedScannerClass) {
                $classScanner = new \Builderius\Zend\Code\Scanner\DerivedClassScanner($classScanner, $this);
            }
    ...
    public function getClass($class, $returnDerivedScannerClass = \false)
    {
        ...
        if (!isset($this->classToFileScanner[$class])) {
            throw new \Builderius\Zend\Code\Exception\InvalidArgumentException('Class not found.');
        }
        /** @var FileScanner $fs */
        $fs = $this->fileScanners[$this->classToFileScanner[$class]];
        $returnClass = $fs->getClass($class);
        if ($returnClass instanceof \Builderius\Zend\Code\Scanner\ClassScanner && $returnDerivedScannerClass) {
            return new \Builderius\Zend\Code\Scanner\DerivedClassScanner($returnClass, $this);
        }
        return $returnClass;
    }
    /**
