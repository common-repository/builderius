...
class Util
{
    ...
    public static function resolveImports(&$value, $key = null, \stdClass $data = null)
    {
        if (!\is_object($data) || !\property_exists($data, 'uses') || !\property_exists($data, 'namespace')) {
            throw new \Builderius\Zend\Code\Exception\InvalidArgumentException(\sprintf('%s expects a data object containing "uses" and "namespace" properties; on or both missing', __METHOD__));
        }
