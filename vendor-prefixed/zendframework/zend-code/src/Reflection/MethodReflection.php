...
class MethodReflection extends \ReflectionMethod implements \Builderius\Zend\Code\Reflection\ReflectionInterface
{
    /**
    ...
    public function getDocBlock()
    {
        ...
        $instance = new \Builderius\Zend\Code\Reflection\DocBlockReflection($this);
        return $instance;
    }
    /**
     ...
     */
    #[\ReturnTypeWillChange]
    public function getPrototype($format = \Builderius\Zend\Code\Reflection\MethodReflection::PROTOTYPE_AS_ARRAY)
    {
        $returnType = 'mixed';
        ...
        if ($format == \Builderius\Zend\Code\Reflection\MethodReflection::PROTOTYPE_AS_STRING) {
    ...
    public function getParameters()
    {
        ...
        while ($phpReflections && ($phpReflection = \array_shift($phpReflections))) {
            $instance = new \Builderius\Zend\Code\Reflection\ParameterReflection([$this->getDeclaringClass()->getName(), $this->getName()], $phpReflection->getName());
    ...
    protected function extractMethodContents($bodyOnly = \false)
    {
        ...
        $tokens = \token_get_all('<?php ' . $functionLine);
        ...
        foreach ($tokens as $key => $token) {
            $tokenType = \is_array($token) ? \token_name($token[0]) : $token;
    ...
    protected function extractPrefixedWhitespace($haystack, $position)
    {
        ...
        for ($i = $position - 1; $i >= 0; $i--) {
            $tokenType = \is_array($haystack[$i]) ? \token_name($haystack[$i][0]) : $haystack[$i];
