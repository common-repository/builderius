...
use Reflector;
use Builderius\Zend\Code\Reflection\DocBlock\Tag\TagInterface as DocBlockTagInterface;
use Builderius\Zend\Code\Reflection\DocBlock\TagManager as DocBlockTagManager;
use Builderius\Zend\Code\Scanner\DocBlockScanner;
use function count;
...
class DocBlockReflection implements \Builderius\Zend\Code\Reflection\ReflectionInterface
{
    /**
     * @var Reflector
     */
     ...
     */
    public function __construct($commentOrReflector, \Builderius\Zend\Code\Reflection\DocBlock\TagManager $tagManager = null)
    {
        if (!$tagManager) {
            $tagManager = new \Builderius\Zend\Code\Reflection\DocBlock\TagManager();
        ...
        if ($commentOrReflector instanceof \Reflector) {
            ...
            if (!\method_exists($commentOrReflector, 'getDocComment')) {
                throw new \Builderius\Zend\Code\Reflection\Exception\InvalidArgumentException('Reflector must contain method "getDocComment"');
            }
            ...
            throw new \Builderius\Zend\Code\Reflection\Exception\InvalidArgumentException(\sprintf('%s must have a (string) DocComment or a Reflector in the constructor', \get_class($this)));
        }
        if ($this->docComment == '') {
            throw new \Builderius\Zend\Code\Reflection\Exception\InvalidArgumentException('DocComment cannot be empty');
        }
        $this->reflect();
    }
    /**
    ...
    protected function reflect()
    {
        ...
        $scanner = new \Builderius\Zend\Code\Scanner\DocBlockScanner($docComment);
