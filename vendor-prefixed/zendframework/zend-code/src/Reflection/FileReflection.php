...
class FileReflection implements \Builderius\Zend\Code\Reflection\ReflectionInterface
{
    /**
     * @var string
     */
    ...
    public function __construct($filename, $includeIfNotAlreadyIncluded = \false)
    {
        ...
        if (!$fileRealPath) {
            throw new \Builderius\Zend\Code\Reflection\Exception\InvalidArgumentException(\sprintf('No file for %s was found.', $filename));
        }
        if (!\in_array($fileRealPath, \get_included_files())) {
            if (!$includeIfNotAlreadyIncluded) {
                throw new \Builderius\Zend\Code\Reflection\Exception\RuntimeException(\sprintf('File %s must be required before it can be reflected', $filename));
            }
    ...
    public function getDocBlock()
    {
        ...
        $instance = new \Builderius\Zend\Code\Reflection\DocBlockReflection($docComment);
        return $instance;
    }
    /**
     * @return string[]
    ...
    public function getClasses()
    {
        ...
        foreach ($this->classes as $class) {
            $classes[] = new \Builderius\Zend\Code\Reflection\ClassReflection($class);
        }
        return $classes;
    }
    /**
    ...
    public function getFunctions()
    {
        ...
        foreach ($this->functions as $function) {
            $functions[] = new \Builderius\Zend\Code\Reflection\FunctionReflection($function);
        }
        return $functions;
    }
    /**
    ...
    public function getClass($name = null)
    {
        if (null === $name) {
            ...
            return new \Builderius\Zend\Code\Reflection\ClassReflection($selected);
        }
        if (\in_array($name, $this->classes)) {
            return new \Builderius\Zend\Code\Reflection\ClassReflection($name);
        }
        throw new \Builderius\Zend\Code\Reflection\Exception\InvalidArgumentException(\sprintf('Class by name %s not found.', $name));
    }
    /**
    ...
    {
        $scanner = new \Builderius\Zend\Code\Scanner\CachingFileScanner($this->filePath);
