...
use Builderius\Zend\Code\Generator\Exception\RuntimeException;
...
class FileGeneratorRegistry
{
     ...
     */
    public static function registerFileCodeGenerator(\Builderius\Zend\Code\Generator\FileGenerator $fileCodeGenerator, $fileName = null)
    {
        if ($fileName === null) {
            $fileName = $fileCodeGenerator->getFilename();
        }
        if ($fileName == '') {
            throw new \Builderius\Zend\Code\Generator\Exception\RuntimeException('FileName does not exist.');
        }
