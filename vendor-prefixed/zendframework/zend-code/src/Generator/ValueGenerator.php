...
use Builderius\Zend\Stdlib\ArrayObject as StdlibArrayObject;
...
use function strpos;
class ValueGenerator extends \Builderius\Zend\Code\Generator\AbstractGenerator
{
    /**#@+
     * Constant values
     */
    ...
    public function __construct($value = null, $type = self::TYPE_AUTO, $outputMode = self::OUTPUT_MULTIPLE_LINE, $constants = null)
    {
        ...
        if ($constants === null) {
            $constants = new \ArrayObject();
        } elseif (!($constants instanceof \ArrayObject || $constants instanceof \Builderius\Zend\Stdlib\ArrayObject)) {
            throw new \Builderius\Zend\Code\Exception\InvalidArgumentException('Builderius\\$constants must be an instance of ArrayObject or Zend\\Stdlib\\ArrayObject');
        }
    ...
    public function generate()
    {
        ...
        switch ($type) {
            ...
            default:
                throw new \Builderius\Zend\Code\Generator\Exception\RuntimeException(\sprintf('Type "%s" is unknown or cannot be used as property default value.', \is_object($value) ? \get_class($value) : \gettype($value)));
        }
        return $output;
    }
    /**
