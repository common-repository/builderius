...
use Builderius\Zend\Code\DeclareStatement;
use Builderius\Zend\Code\Exception\InvalidArgumentException;
use Builderius\Zend\Code\Reflection\Exception as ReflectionException;
use Builderius\Zend\Code\Reflection\FileReflection;
...
class FileGenerator extends \Builderius\Zend\Code\Generator\AbstractGenerator
{
    /**
     * @var string
     */
    ...
    {
        $fileReflector = new \Builderius\Zend\Code\Reflection\FileReflection($filePath, $includeIfNotAlreadyIncluded);
     ...
     * @return FileGenerator
     */
    public static function fromReflection(\Builderius\Zend\Code\Reflection\FileReflection $fileReflection)
    {
        $file = new static();
        ...
        foreach ($fileReflection->getClasses() as $class) {
            $phpClass = \Builderius\Zend\Code\Generator\ClassGenerator::fromReflection($class);
        ...
        if ($fileReflection->getDocComment() != '') {
            ...
            $file->setDocBlock(\Builderius\Zend\Code\Generator\DocBlockGenerator::fromReflection($docBlock));
        }
        return $file;
    }
    /**
        ...
        foreach ($values as $name => $value) {
            switch (\strtolower(\str_replace(['.', '-', '_'], '', $name))) {
                case 'filename':
                    ...
                    break;
                case 'class':
                    $fileGenerator->setClass($value instanceof \Builderius\Zend\Code\Generator\ClassGenerator ? $value : \Builderius\Zend\Code\Generator\ClassGenerator::fromArray($value));
                    break;
                case 'requiredfiles':
                ...
                case 'declares':
                    $fileGenerator->setDeclares(\array_map(static function ($directive, $value) {
                        return \Builderius\Zend\Code\DeclareStatement::fromArray([$directive => $value]);
    ...
    public function setDocBlock($docBlock)
    {
        ...
        if (\is_array($docBlock)) {
            $docBlock = new \Builderius\Zend\Code\Generator\DocBlockGenerator($docBlock);
        } elseif (!$docBlock instanceof \Builderius\Zend\Code\Generator\DocBlockGenerator) {
            throw new \Builderius\Zend\Code\Generator\Exception\InvalidArgumentException(\sprintf('Builderius\\%s is expecting either a string, array or an instance of %s\\DocBlockGenerator', __METHOD__, __NAMESPACE__));
        }
    ...
    {
        if (\is_array($class)) {
            $class = \Builderius\Zend\Code\Generator\ClassGenerator::fromArray($class);
        } elseif (\is_string($class)) {
            $class = new \Builderius\Zend\Code\Generator\ClassGenerator($class);
        } elseif (!$class instanceof \Builderius\Zend\Code\Generator\ClassGenerator) {
            throw new \Builderius\Zend\Code\Generator\Exception\InvalidArgumentException(\sprintf('Builderius\\%s is expecting either a string, array or an instance of %s\\ClassGenerator', __METHOD__, __NAMESPACE__));
        }
    ...
    public function setDeclares(array $declares)
    {
        foreach ($declares as $declare) {
            if (!$declare instanceof \Builderius\Zend\Code\DeclareStatement) {
                throw new \Builderius\Zend\Code\Exception\InvalidArgumentException(\sprintf('%s is expecting an array of %s objects', __METHOD__, \Builderius\Zend\Code\DeclareStatement::class));
            }
    ...
    public function write()
    {
        if ($this->filename == '' || !\is_writable(\dirname($this->filename))) {
            throw new \Builderius\Zend\Code\Generator\Exception\RuntimeException('This code generator object is not writable.');
        }
