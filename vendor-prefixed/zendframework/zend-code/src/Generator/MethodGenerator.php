...
use function substr;
use function trim;
class MethodGenerator extends \Builderius\Zend\Code\Generator\AbstractMemberGenerator
{
    /**
     ...
     */
    public static function fromReflection(\Builderius\Zend\Code\Reflection\MethodReflection $reflectionMethod)
    {
        $method = static::copyMethodSignature($reflectionMethod);
        $method->setSourceContent($reflectionMethod->getContents(\false));
        $method->setSourceDirty(\false);
        if ($reflectionMethod->getDocComment() != '') {
            $method->setDocBlock(\Builderius\Zend\Code\Generator\DocBlockGenerator::fromReflection($reflectionMethod->getDocBlock()));
        }
     ...
     */
    public static function copyMethodSignature(\Builderius\Zend\Code\Reflection\MethodReflection $reflectionMethod) : \Builderius\Zend\Code\Generator\MethodGenerator
    {
        $method = new static();
        ...
        foreach ($reflectionMethod->getParameters() as $reflectionParameter) {
            $method->setParameter(\Builderius\Zend\Code\Generator\ParameterGenerator::fromReflection($reflectionParameter));
        }
        return $method;
    }
    /**
    ...
    {
        if (!isset($array['name'])) {
            throw new \Builderius\Zend\Code\Generator\Exception\InvalidArgumentException('Method generator requires that a name is provided for this object');
        }
    ...
    public function setParameter($parameter)
    {
        if (\is_string($parameter)) {
            $parameter = new \Builderius\Zend\Code\Generator\ParameterGenerator($parameter);
        }
        if (\is_array($parameter)) {
            $parameter = \Builderius\Zend\Code\Generator\ParameterGenerator::fromArray($parameter);
        }
        if (!$parameter instanceof \Builderius\Zend\Code\Generator\ParameterGenerator) {
            throw new \Builderius\Zend\Code\Generator\Exception\InvalidArgumentException(\sprintf('Builderius\\%s is expecting either a string, array or an instance of %s\\ParameterGenerator', __METHOD__, __NAMESPACE__));
        }
    ...
    {
        $this->returnType = null === $returnType ? null : \Builderius\Zend\Code\Generator\TypeGenerator::fromTypeString($returnType);
        return $this;
    }
    /**
    ...
    public function generate()
    {
        ...
        if ($this->body) {
            $output .= \preg_replace('#^((?![a-zA-Z0-9_-]+;).+?)$#m', $indent . $indent . '$1', \trim($this->body)) . self::LINE_FEED;
        }
     ...
     * @return null|string
     */
    private static function extractReturnTypeFromMethodReflection(\Builderius\Zend\Code\Reflection\MethodReflection $methodReflection)
    {
