...
use function strpos;
class TraitUsageGenerator extends \Builderius\Zend\Code\Generator\AbstractGenerator implements \Builderius\Zend\Code\Generator\TraitUsageInterface
{
    /**
     * @var ClassGenerator
     */
     ...
     */
    protected $uses = [];
    public function __construct(\Builderius\Zend\Code\Generator\ClassGenerator $classGenerator)
    {
    ...
    public function addTrait($trait)
    {
        ...
        if (\is_array($trait)) {
            if (!\array_key_exists('traitName', $trait)) {
                throw new \Builderius\Zend\Code\Generator\Exception\InvalidArgumentException('Missing required value for traitName');
            }
    ...
    public function addTraitAlias($method, $alias, $visibility = null)
    {
        ...
        if (\is_array($method)) {
            if (!\array_key_exists('traitName', $method)) {
                throw new \Builderius\Zend\Code\Generator\Exception\InvalidArgumentException('Missing required argument "traitName" for $method');
            }
            if (!\array_key_exists('method', $method)) {
                throw new \Builderius\Zend\Code\Generator\Exception\InvalidArgumentException('Missing required argument "method" for $method');
            }
            $traitAndMethod = $method['traitName'] . '::' . $method['method'];
        }
        // Validations
        if (\false === \strpos($traitAndMethod, '::')) {
            throw new \Builderius\Zend\Code\Generator\Exception\InvalidArgumentException('Invalid Format: $method must be in the format of trait::method');
        }
        if (!\is_string($alias)) {
            throw new \Builderius\Zend\Code\Generator\Exception\InvalidArgumentException('Invalid Alias: $alias must be a string or array.');
        }
        if ($this->classGenerator->hasMethod($alias)) {
            throw new \Builderius\Zend\Code\Generator\Exception\InvalidArgumentException('Invalid Alias: Method name already exists on this class.');
        }
        if (null !== $visibility && $visibility !== \ReflectionMethod::IS_PUBLIC && $visibility !== \ReflectionMethod::IS_PRIVATE && $visibility !== \ReflectionMethod::IS_PROTECTED) {
            throw new \Builderius\Zend\Code\Generator\Exception\InvalidArgumentException('Invalid Type: $visibility must of ReflectionMethod::IS_PUBLIC,' . ' ReflectionMethod::IS_PRIVATE or ReflectionMethod::IS_PROTECTED');
        }
        list($trait, $method) = \explode('::', $traitAndMethod);
        if (!$this->hasTrait($trait)) {
            throw new \Builderius\Zend\Code\Generator\Exception\InvalidArgumentException('Invalid trait: Trait does not exists on this class');
        }
    ...
    public function addTraitOverride($method, $traitsToReplace)
    {
        ...
        foreach ($traitsToReplace as $traitToReplace) {
            if (!\is_string($traitToReplace)) {
                throw new \Builderius\Zend\Code\Generator\Exception\InvalidArgumentException('Invalid Argument: $traitToReplace must be a string or array of strings');
            }
    ...
    public function generate()
    {
        ...
        foreach ($aliases as $method => $alias) {
            ...
            if ($this->classGenerator->hasMethod($alias['alias'])) {
                throw new \Builderius\Zend\Code\Generator\Exception\RuntimeException(\sprintf('Generation Error: Aliased method %s already exists on this class', $alias['alias']));
            }
