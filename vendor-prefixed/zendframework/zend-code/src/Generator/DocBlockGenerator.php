...
class DocBlockGenerator extends \Builderius\Zend\Code\Generator\AbstractGenerator
{
    /**
     * @var string
     */
     ...
     */
    public static function fromReflection(\Builderius\Zend\Code\Reflection\DocBlockReflection $reflectionDocBlock)
    {
        $docBlock = new static();
    ...
    protected static function getTagManager()
    {
        if (!isset(static::$tagManager)) {
            static::$tagManager = new \Builderius\Zend\Code\Generator\DocBlock\TagManager();
    ...
    public function setTag($tag)
    {
        if (\is_array($tag)) {
            ...
            $genericTag = new \Builderius\Zend\Code\Generator\DocBlock\Tag();
            $genericTag->setOptions($tag);
            $tag = $genericTag;
        } elseif (!$tag instanceof \Builderius\Zend\Code\Generator\DocBlock\Tag\TagInterface) {
            throw new \Builderius\Zend\Code\Generator\Exception\InvalidArgumentException(\sprintf('Builderius\\%s expects either an array of method options or an instance of %s\\DocBlock\\Tag\\TagInterface', __METHOD__, __NAMESPACE__));
        }
        $this->tags[] = $tag;
        return $this;
    }
    ...
    public function generate()
    {
        if (!$this->isSourceDirty()) {
            return $this->docCommentize(\trim($this->getSourceContent()));
        }
        $output = '';
        ...
        return $this->docCommentize(\trim($output));
    }
    /**
