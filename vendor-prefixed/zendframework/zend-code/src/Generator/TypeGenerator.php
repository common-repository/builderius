...
use Builderius\Zend\Code\Generator\Exception\InvalidArgumentException;
...
use function substr;
final class TypeGenerator implements \Builderius\Zend\Code\Generator\GeneratorInterface
{
    /**
     * @var bool
     */
    ...
    public static function fromTypeString($type)
    {
        ...
        if (!\preg_match(self::$validIdentifierMatcher, $trimmedType)) {
            throw new \Builderius\Zend\Code\Generator\Exception\InvalidArgumentException(\sprintf('Provided type "%s" is invalid: must conform "%s"', $type, self::$validIdentifierMatcher));
        }
        $isInternalPhpType = self::isInternalPhpType($trimmedType);
        if ($wasTrimmed && $isInternalPhpType) {
            throw new \Builderius\Zend\Code\Generator\Exception\InvalidArgumentException(\sprintf('Provided type "%s" is an internal PHP type, but was provided with a namespace separator prefix', $type));
        }
        if ($nullable && $isInternalPhpType && 'void' === \strtolower($trimmedType)) {
            throw new \Builderius\Zend\Code\Generator\Exception\InvalidArgumentException(\sprintf('Provided type "%s" cannot be nullable', $type));
        }
        $instance = new self();
