 ...
 */
final class ContainsAny extends \Builderius\Respect\Validation\Rules\AbstractEnvelope
{
    /**
    ...
    {
        parent::__construct(new \Builderius\Respect\Validation\Rules\AnyOf(...$this->getRules($needles, $identical)), ['needles' => $needles]);
    }
    /**
     * @param mixed[] $needles
     *
    ...
    private function getRules(array $needles, bool $identical) : array
    {
        return \array_map(static function ($needle) use($identical) : Contains {
            return new \Builderius\Respect\Validation\Rules\Contains($needle, $identical);
        }, $needles);
    }
}
