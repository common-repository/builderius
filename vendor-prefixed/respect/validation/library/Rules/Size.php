...
use Builderius\Psr\Http\Message\UploadedFileInterface;
 ...
 */
final class Size extends \Builderius\Respect\Validation\Rules\AbstractRule
{
    /**
     * @var string|int|null
     */
    ...
    public function validate($input) : bool
    {
        ...
        if ($input instanceof \Builderius\Psr\Http\Message\UploadedFileInterface) {
            return $this->isValidSize($input->getSize());
        }
    ...
    private function toBytes($size) : float
    {
        ...
        if (!\is_numeric($value)) {
            throw new \Builderius\Respect\Validation\Exceptions\ComponentException(\sprintf('"%s" is not a recognized file size.', (string) $size));
        }
        return (float) $value;
    }
