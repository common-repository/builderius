...
use Builderius\Symfony\Component\Cache\Traits\FilesystemCommonTrait;
 ...
 */
class PhpFilesAdapter extends \Builderius\Symfony\Component\Cache\Adapter\AbstractAdapter implements \Builderius\Symfony\Component\Cache\PruneableInterface
{
    ...
    protected function doFetch(array $ids)
    {
        ...
        foreach ($ids as $id) {
            if (null === ($value = $this->values[$id] ?? null)) {
                ...
                $values[$id] = $value;
            } elseif (!$value instanceof \Builderius\Symfony\Component\Cache\Adapter\LazyValue) {
                $values[$id] = $value();
            ...
            foreach ($missingIds as $k => $id) {
                try {
                    ...
                    if (isset(self::$valuesCache[$file])) {
                        ...
                        $this->values[$id] = new \Builderius\Symfony\Component\Cache\Adapter\LazyValue($file);
                    }
                    if ($now >= $expiresAt) {
    ...
    protected function doHave(string $id)
    {
        ...
        try {
            ...
            if (isset(self::$valuesCache[$file])) {
                ...
                $value = new \Builderius\Symfony\Component\Cache\Adapter\LazyValue($file);
            }
        } catch (\ErrorException $e) {
            return \false;
        } finally {
    ...
    protected function doSave(array $values, int $lifetime)
    {
        ...
        if (!$ok && !\is_writable($this->directory)) {
            throw new \Builderius\Symfony\Component\Cache\Exception\CacheException(\sprintf('Cache directory is not writable (%s).', $this->directory));
        }
        return $ok;
    }
    /**
