 ...
 */
class TagAwareAdapter implements \Builderius\Symfony\Component\Cache\Adapter\TagAwareAdapterInterface, \Builderius\Symfony\Contracts\Cache\TagAwareCacheInterface, \Builderius\Symfony\Component\Cache\PruneableInterface, \Builderius\Symfony\Component\Cache\ResettableInterface
{
    ...
    public function __construct(\Builderius\Symfony\Component\Cache\Adapter\AdapterInterface $itemsPool, \Builderius\Symfony\Component\Cache\Adapter\AdapterInterface $tagsPool = null, float $knownTagVersionsTtl = 0.15)
    {
        $this->pool = $itemsPool;
        $this->tags = $tagsPool ?: $itemsPool;
        $this->knownTagVersionsTtl = $knownTagVersionsTtl;
        $this->createCacheItem = \Closure::bind(static function ($key, $value, \Builderius\Symfony\Component\Cache\CacheItem $protoItem) {
        ...
        $this->setCacheItemTags = \Closure::bind(static function (\Builderius\Symfony\Component\Cache\CacheItem $item, $key, array &$itemTags) {
            $item->isTaggable = \true;
            if (!$item->isHit) {
                return $item;
            }
            if (isset($itemTags[$key])) {
                foreach ($itemTags[$key] as $tag => $version) {
                    $item->metadata[\Builderius\Symfony\Component\Cache\CacheItem::METADATA_TAGS][$tag] = $tag;
                }
                unset($itemTags[$key]);
            } else {
                $item->value = null;
        ...
        $this->getTagsByKey = \Closure::bind(static function ($deferred) {
            ...
            foreach ($deferred as $key => $item) {
                $tagsByKey[$key] = $item->newMetadata[\Builderius\Symfony\Component\Cache\CacheItem::METADATA_TAGS] ?? [];
            }
            return $tagsByKey;
        }, null, \Builderius\Symfony\Component\Cache\CacheItem::class);
        $this->invalidateTags = \Closure::bind(static function (\Builderius\Symfony\Component\Cache\Adapter\AdapterInterface $tagsAdapter, array $tags) {
            foreach ($tags as $v) {
                $v->expiry = 0;
    ...
    public function invalidateTags(array $tags)
    {
        ...
        $invalidatedTags = [];
        foreach ($tags as $tag) {
            \Builderius\Symfony\Component\Cache\CacheItem::validateKey($tag);
    ...
    public function getItems(array $keys = [])
    {
        ...
        } catch (\Builderius\Psr\Cache\InvalidArgumentException $e) {
