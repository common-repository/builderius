...
use Builderius\Predis\Connection\Aggregate\PredisCluster;
...
use Builderius\Symfony\Component\Cache\Marshaller\DeflateMarshaller;
use Builderius\Symfony\Component\Cache\Marshaller\MarshallerInterface;
use Builderius\Symfony\Component\Cache\Marshaller\TagAwareMarshaller;
use Builderius\Symfony\Component\Cache\Traits\RedisTrait;
/**
 ...
 */
class RedisTagAwareAdapter extends \Builderius\Symfony\Component\Cache\Adapter\AbstractTagAwareAdapter
{
    use RedisTrait;
    /**
     ...
     */
    public function __construct($redisClient, string $namespace = '', int $defaultLifetime = 0, \Builderius\Symfony\Component\Cache\Marshaller\MarshallerInterface $marshaller = null)
    {
        if ($redisClient instanceof \Builderius\Predis\ClientInterface && $redisClient->getConnection() instanceof \Builderius\Predis\Connection\Aggregate\ClusterInterface && !$redisClient->getConnection() instanceof \Builderius\Predis\Connection\Aggregate\PredisCluster) {
            throw new \Builderius\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Unsupported Predis cluster connection: only "%s" is, "%s" given.', \Builderius\Predis\Connection\Aggregate\PredisCluster::class, \get_class($redisClient->getConnection())));
        }
        if (\defined('Redis::OPT_COMPRESSION') && ($redisClient instanceof \Redis || $redisClient instanceof \RedisArray || $redisClient instanceof \RedisCluster)) {
            ...
            foreach (\is_array($compression) ? $compression : [$compression] as $c) {
                if (\Redis::COMPRESSION_NONE !== $c) {
                    throw new \Builderius\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('phpredis compression must be disabled when using "%s", use "%s" instead.', static::class, \Builderius\Symfony\Component\Cache\Marshaller\DeflateMarshaller::class));
                }
            }
        }
        $this->init($redisClient, $namespace, $defaultLifetime, new \Builderius\Symfony\Component\Cache\Marshaller\TagAwareMarshaller($marshaller));
    }
    /**
     * {@inheritdoc}
     */
    protected function doSave(array $values, int $lifetime, array $addTagData = [], array $delTagData = []) : array
    {
        ...
        if ('noeviction' !== $eviction && 0 !== \strpos($eviction, 'volatile-')) {
            throw new \Builderius\Symfony\Component\Cache\Exception\LogicException(\sprintf('Redis maxmemory-policy setting "%s" is *not* supported by RedisTagAwareAdapter, use "noeviction" or  "volatile-*" eviction policies.', $eviction));
        }
        // serialize values
...
EOLUA;
    ...
    {
        if (!$this->redis instanceof \Builderius\Predis\ClientInterface || !$this->redis->getConnection() instanceof \Builderius\Predis\Connection\Aggregate\PredisCluster) {
