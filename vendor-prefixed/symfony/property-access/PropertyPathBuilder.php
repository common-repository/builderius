...
class PropertyPathBuilder
{
    ...
    {
        if (\is_string($path)) {
            $path = new \Builderius\Symfony\Component\PropertyAccess\PropertyPath($path);
        }
        if (0 === $length) {
            $end = $path->getLength();
        } else {
    ...
    public function remove(int $offset, int $length = 1)
    {
        if (!isset($this->elements[$offset])) {
            throw new \Builderius\Symfony\Component\PropertyAccess\Exception\OutOfBoundsException(\sprintf('The offset "%s" is not within the property path.', $offset));
        }
    ...
    public function replace(int $offset, int $length, $path, int $pathOffset = 0, int $pathLength = 0)
    {
        ...
        if ($offset < 0 && \abs($offset) <= $this->getLength()) {
            ...
            throw new \Builderius\Symfony\Component\PropertyAccess\Exception\OutOfBoundsException('The offset ' . $offset . ' is not within the property path');
        }
        if (0 === $pathLength) {
    ...
    public function getPropertyPath()
    {
        ...
        return '' !== $pathAsString ? new \Builderius\Symfony\Component\PropertyAccess\PropertyPath($pathAsString) : null;
    }
    /**
