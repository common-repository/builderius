...
use Builderius\Psr\Log\NullLogger;
use Builderius\Symfony\Component\Cache\Adapter\AdapterInterface;
use Builderius\Symfony\Component\Cache\Adapter\ApcuAdapter;
use Builderius\Symfony\Component\Cache\Adapter\NullAdapter;
use Builderius\Symfony\Component\Inflector\Inflector;
use Builderius\Symfony\Component\PropertyAccess\Exception\AccessException;
use Builderius\Symfony\Component\PropertyAccess\Exception\InvalidArgumentException;
use Builderius\Symfony\Component\PropertyAccess\Exception\NoSuchIndexException;
use Builderius\Symfony\Component\PropertyAccess\Exception\NoSuchPropertyException;
use Builderius\Symfony\Component\PropertyAccess\Exception\UnexpectedTypeException;
/**
 ...
 */
class PropertyAccessor implements \Builderius\Symfony\Component\PropertyAccess\PropertyAccessorInterface
{
     ...
     */
    public function __construct(bool $magicCall = \false, bool $throwExceptionOnInvalidIndex = \false, \Builderius\Psr\Cache\CacheItemPoolInterface $cacheItemPool = null, bool $throwExceptionOnInvalidPropertyPath = \true)
    {
        $this->magicCall = $magicCall;
        $this->ignoreInvalidIndices = !$throwExceptionOnInvalidIndex;
        $this->cacheItemPool = $cacheItemPool instanceof \Builderius\Symfony\Component\Cache\Adapter\NullAdapter ? null : $cacheItemPool;
    ...
    private static function throwInvalidArgumentException(string $message, array $trace, int $i, string $propertyPath, \Throwable $previous = null) : void
    {
        ...
        if (\PHP_VERSION_ID < 80000) {
            ...
            throw new \Builderius\Symfony\Component\PropertyAccess\Exception\InvalidArgumentException(\sprintf('Expected argument of type "%s", "%s" given at property path "%s".', $message, 'NULL' === $type ? 'null' : $type, $propertyPath), 0, $previous);
        }
        if (\preg_match('/^\\S+::\\S+\\(\\): Argument #\\d+ \\(\\$\\S+\\) must be of type (\\S+), (\\S+) given/', $message, $matches)) {
            list(, $expectedType, $actualType) = $matches;
            throw new \Builderius\Symfony\Component\PropertyAccess\Exception\InvalidArgumentException(\sprintf('Expected argument of type "%s", "%s" given at property path "%s".', $expectedType, 'NULL' === $actualType ? 'null' : $actualType, $propertyPath), 0, $previous);
        }
    }
    /**
     * {@inheritdoc}
    ...
    {
        if (!$propertyPath instanceof \Builderius\Symfony\Component\PropertyAccess\PropertyPathInterface) {
            $propertyPath = new \Builderius\Symfony\Component\PropertyAccess\PropertyPath($propertyPath);
        }
        try {
            $zval = [self::VALUE => $objectOrArray];
            $this->readPropertiesUntil($zval, $propertyPath, $propertyPath->getLength(), $this->ignoreInvalidIndices);
            return \true;
        } catch (\Builderius\Symfony\Component\PropertyAccess\Exception\AccessException $e) {
            return \false;
        } catch (\Builderius\Symfony\Component\PropertyAccess\Exception\UnexpectedTypeException $e) {
            return \false;
        }
    }
    /**
     ...
     */
    private function readPropertiesUntil(array $zval, \Builderius\Symfony\Component\PropertyAccess\PropertyPathInterface $propertyPath, int $lastIndex, bool $ignoreInvalidIndices = \true) : array
    {
        if (!\is_object($zval[self::VALUE]) && !\is_array($zval[self::VALUE])) {
            throw new \Builderius\Symfony\Component\PropertyAccess\Exception\UnexpectedTypeException($zval[self::VALUE], $propertyPath, 0);
        }
        ...
        for ($i = 0; $i < $lastIndex; ++$i) {
                    ...
                    if (!$ignoreInvalidIndices) {
                        if (!\is_array($zval[self::VALUE])) {
                            if (!$zval[self::VALUE] instanceof \Traversable) {
                                throw new \Builderius\Symfony\Component\PropertyAccess\Exception\NoSuchIndexException(\sprintf('Cannot read index "%s" while trying to traverse path "%s".', $property, (string) $propertyPath));
                            }
                            $zval[self::VALUE] = \iterator_to_array($zval[self::VALUE]);
                        }
                        throw new \Builderius\Symfony\Component\PropertyAccess\Exception\NoSuchIndexException(\sprintf('Cannot read index "%s" while trying to traverse path "%s". Available indices are "%s".', $property, (string) $propertyPath, \print_r(\array_keys($zval[self::VALUE]), \true)));
                    }
            ...
            if ($i + 1 < $propertyPath->getLength() && !\is_object($zval[self::VALUE]) && !\is_array($zval[self::VALUE])) {
                throw new \Builderius\Symfony\Component\PropertyAccess\Exception\UnexpectedTypeException($zval[self::VALUE], $propertyPath, $i + 1);
            }
    ...
    {
        if (!$zval[self::VALUE] instanceof \ArrayAccess && !\is_array($zval[self::VALUE])) {
            throw new \Builderius\Symfony\Component\PropertyAccess\Exception\NoSuchIndexException(\sprintf('Cannot read index "%s" from object of type "%s" because it doesn\'t implement \\ArrayAccess.', $index, \get_class($zval[self::VALUE])));
        }
    ...
    {
        if (!\is_object($zval[self::VALUE])) {
            throw new \Builderius\Symfony\Component\PropertyAccess\Exception\NoSuchPropertyException(\sprintf('Cannot read property "%s" from an array. Maybe you intended to write the property path as "[%1$s]" instead.', $property));
        }
        $result = self::$resultProto;
        ...
        try {
            if (self::ACCESS_TYPE_METHOD === $access[self::ACCESS_TYPE]) {
                try {
                    ...
                    if (__FILE__ === $trace['file'] && $access[self::ACCESS_NAME] === $trace['function'] && $object instanceof $trace['class'] && \preg_match(\sprintf('/Return value (?:of .*::\\w+\\(\\) )?must be of (?:the )?type (\\w+), null returned$/'), $e->getMessage(), $matches)) {
                        throw new \Builderius\Symfony\Component\PropertyAccess\Exception\AccessException(\sprintf('The method "%s::%s()" returned "null", but expected type "%3$s". Did you forget to initialize a property or to make the return type nullable using "?%3$s"?', \false === \strpos(\get_class($object), "@anonymous\0") ? \get_class($object) : (\get_parent_class($object) ?: 'class') . '@anonymous', $access[self::ACCESS_NAME], $matches[1]), 0, $e);
                    }
                    throw $e;
                }
                ...
                throw new \Builderius\Symfony\Component\PropertyAccess\Exception\NoSuchPropertyException($access[self::ACCESS_NAME]);
            }
        } catch (\Error $e) {
            ...
            if (\PHP_VERSION_ID >= 70400 && \preg_match('/^Typed property ([\\w\\\\]+)::\\$(\\w+) must not be accessed before initialization$/', $e->getMessage(), $matches)) {
                ...
                throw new \Builderius\Symfony\Component\PropertyAccess\Exception\AccessException(\sprintf('The property "%s::$%s" is not readable because it is typed "%s". You should initialize it or declare a default value instead.', $r->getDeclaringClass()->getName(), $r->getName(), $type), 0, $e);
            }
            throw $e;
        }
    ...
    {
        if (!$zval[self::VALUE] instanceof \ArrayAccess && !\is_array($zval[self::VALUE])) {
            throw new \Builderius\Symfony\Component\PropertyAccess\Exception\NoSuchIndexException(\sprintf('Cannot modify index "%s" in object of type "%s" because it doesn\'t implement \\ArrayAccess.', $index, \get_class($zval[self::VALUE])));
        }
    ...
    {
        if (!\is_object($zval[self::VALUE])) {
            throw new \Builderius\Symfony\Component\PropertyAccess\Exception\NoSuchPropertyException(\sprintf('Cannot write property "%s" to an array. Maybe you should write the property path as "[%1$s]" instead?', $property));
        }
        $object = $zval[self::VALUE];
        ...
        if (self::ACCESS_TYPE_METHOD === $access[self::ACCESS_TYPE]) {
            ...
            throw new \Builderius\Symfony\Component\PropertyAccess\Exception\NoSuchPropertyException(\sprintf('Could not determine access type for property "%s" in class "%s"%s.', $property, \get_class($object), isset($access[self::ACCESS_NAME]) ? ': ' . $access[self::ACCESS_NAME] : '.'));
        } else {
            throw new \Builderius\Symfony\Component\PropertyAccess\Exception\NoSuchPropertyException($access[self::ACCESS_NAME]);
        }
    }
    ...
    private function getWriteAccessInfo(string $class, string $property, $value) : array
    {
        ...
        $singulars = (array) \Builderius\Symfony\Component\Inflector\Inflector::singularize($camelized);
        $errors = [];
     ...
     */
    private function getPropertyPath($propertyPath) : \Builderius\Symfony\Component\PropertyAccess\PropertyPath
    {
        if ($propertyPath instanceof \Builderius\Symfony\Component\PropertyAccess\PropertyPathInterface) {
        ...
        $propertyPathInstance = new \Builderius\Symfony\Component\PropertyAccess\PropertyPath($propertyPath);
        if (isset($item)) {
     ...
     */
    public static function createCache(string $namespace, int $defaultLifetime, string $version, \Builderius\Psr\Log\LoggerInterface $logger = null)
    {
        if (!\class_exists('Builderius\\Symfony\\Component\\Cache\\Adapter\\ApcuAdapter')) {
            throw new \LogicException(\sprintf('The Symfony Cache component must be installed to use "%s()".', __METHOD__));
        }
        if (!\Builderius\Symfony\Component\Cache\Adapter\ApcuAdapter::isSupported()) {
            return new \Builderius\Symfony\Component\Cache\Adapter\NullAdapter();
        }
        $apcu = new \Builderius\Symfony\Component\Cache\Adapter\ApcuAdapter($namespace, $defaultLifetime / 5, $version);
        if ('cli' === \PHP_SAPI && !\filter_var(\ini_get('apc.enable_cli'), \FILTER_VALIDATE_BOOLEAN)) {
            $apcu->setLogger(new \Builderius\Psr\Log\NullLogger());
        } elseif (null !== $logger) {
