...
use Builderius\Symfony\Component\String\Exception\RuntimeException;
/**
 ...
 * @experimental in 5.0
 */
class ByteString extends \Builderius\Symfony\Component\String\AbstractString
{
    ...
    public function match(string $regexp, int $flags = 0, int $offset = 0) : array
    {
        ...
        \set_error_handler(static function ($t, $m) {
            throw new \Builderius\Symfony\Component\String\Exception\InvalidArgumentException($m);
        });
        try {
            if (\false === $match($regexp, $this->string, $matches, $flags | \PREG_UNMATCHED_AS_NULL, $offset)) {
                ...
                foreach (\get_defined_constants(\true)['pcre'] as $k => $v) {
                    if ($lastError === $v && '_ERROR' === \substr($k, -6)) {
                        throw new \Builderius\Symfony\Component\String\Exception\RuntimeException('Matching failed with ' . $k . '.');
                    }
                }
                throw new \Builderius\Symfony\Component\String\Exception\RuntimeException('Matching failed with unknown error code.');
            }
        } finally {
    ...
    public function snake() : parent
    {
        ...
        $str->string = \strtolower(\preg_replace(['/([A-Z]+)([A-Z][a-z])/', '/([a-z\\d])([A-Z])/'], 'Builderius\\1_\\2', $str->string));
        return $str;
    }
    ...
    public function toUnicodeString(string $fromEncoding = null) : \Builderius\Symfony\Component\String\UnicodeString
    {
        return new \Builderius\Symfony\Component\String\UnicodeString($this->toCodePointString($fromEncoding)->string);
    }
    public function toCodePointString(string $fromEncoding = null) : \Builderius\Symfony\Component\String\CodePointString
    {
        $u = new \Builderius\Symfony\Component\String\CodePointString();
        ...
        try {
            ...
            } catch (\Builderius\Symfony\Component\String\Exception\InvalidArgumentException $e) {
        ...
        if (!$validEncoding) {
            throw new \Builderius\Symfony\Component\String\Exception\InvalidArgumentException(\sprintf('Invalid "%s" string.', $fromEncoding ?? 'Windows-1252'));
        }
    ...
    public function width(bool $ignoreAnsiDecoration = \true) : int
    {
        ...
        foreach (\explode("\n", $s) as $s) {
            ...
            $w = \substr_count($s, "­") - \substr_count($s, "\10");
            if ($width < ($w += \strlen($s))) {
                $width = $w;
            }
        }
