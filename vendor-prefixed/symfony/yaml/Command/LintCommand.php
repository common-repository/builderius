 ...
 */
namespace Builderius\Symfony\Component\Yaml\Command;
...
class LintCommand extends \Builderius\Symfony\Component\Console\Command\Command
{
    ...
    {
        $this->setDescription('Lints a file and outputs encountered errors')->addArgument('filename', \Builderius\Symfony\Component\Console\Input\InputArgument::IS_ARRAY, 'A file, a directory or "-" for reading from STDIN')->addOption('format', null, \Builderius\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'The output format', 'txt')->addOption('parse-tags', null, \Builderius\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Parse custom tags')->setHelp(<<<EOF
...
EOF
    ...
    protected function execute(\Builderius\Symfony\Component\Console\Input\InputInterface $input, \Builderius\Symfony\Component\Console\Output\OutputInterface $output)
    {
        ...
        $flags = $input->getOption('parse-tags') ? \Builderius\Symfony\Component\Yaml\Yaml::PARSE_CUSTOM_TAGS : 0;
        if (['-'] === $filenames) {
            return $this->display($io, [$this->validate(\file_get_contents('php://stdin'), $flags)]);
        }
        if (!$filenames) {
            throw new \Builderius\Symfony\Component\Console\Exception\RuntimeException('Please provide a filename or pipe file content to STDIN.');
        }
        $filesInfo = [];
    ...
    private function validate(string $content, int $flags, string $file = null)
    {
        $prevErrorHandler = \set_error_handler(function ($level, $message, $file, $line) use(&$prevErrorHandler) {
            if (\E_USER_DEPRECATED === $level) {
                throw new \Builderius\Symfony\Component\Yaml\Exception\ParseException($message, $this->getParser()->getRealCurrentLineNb() + 1);
            }
            return $prevErrorHandler ? $prevErrorHandler($level, $message, $file, $line) : \false;
        });
        try {
            $this->getParser()->parse($content, \Builderius\Symfony\Component\Yaml\Yaml::PARSE_CONSTANT | $flags);
    ...
    private function display(\Builderius\Symfony\Component\Console\Style\SymfonyStyle $io, array $files) : int
    {
        switch ($this->format) {
            case 'txt':
            ...
            default:
                throw new \Builderius\Symfony\Component\Console\Exception\InvalidArgumentException(\sprintf('The format "%s" is not supported.', $this->format));
        }
    }
    private function displayTxt(\Builderius\Symfony\Component\Console\Style\SymfonyStyle $io, array $filesInfo) : int
    {
    ...
    private function displayJson(\Builderius\Symfony\Component\Console\Style\SymfonyStyle $io, array $filesInfo) : int
    {
        $errors = 0;
    ...
    private function getParser() : \Builderius\Symfony\Component\Yaml\Parser
    {
        if (!$this->parser) {
            $this->parser = new \Builderius\Symfony\Component\Yaml\Parser();
        }
        return $this->parser;
    }
