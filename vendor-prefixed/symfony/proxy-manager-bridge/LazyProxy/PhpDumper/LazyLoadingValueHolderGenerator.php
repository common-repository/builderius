...
use Builderius\ProxyManager\ProxyGenerator\LazyLoadingValueHolderGenerator as BaseGenerator;
use Builderius\Symfony\Component\DependencyInjection\Definition;
use Builderius\Zend\Code\Generator\ClassGenerator;
/**
 * @internal
 */
class LazyLoadingValueHolderGenerator extends \Builderius\ProxyManager\ProxyGenerator\LazyLoadingValueHolderGenerator
{
     ...
     * {@inheritdoc}
     */
    public function generate(\ReflectionClass $originalClass, \Builderius\Zend\Code\Generator\ClassGenerator $classGenerator) : void
    {
        parent::generate($originalClass, $classGenerator);
        foreach ($classGenerator->getMethods() as $method) {
            $body = \preg_replace('/(\\$this->initializer[0-9a-f]++) && \\1->__invoke\\(\\$this->(valueHolder[0-9a-f]++), (.*?), \\1\\);/', '$1 && ($1->__invoke(\\$$2, $3, $1) || 1) && $this->$2 = \\$$2;', $method->getBody());
            ...
            if ($originalClass->isInterface()) {
                $body = \str_replace('get_parent_class($this)', \var_export($originalClass->name, \true), $body);
            ...
            if (0 === \strpos($originalClass->getFilename(), __FILE__)) {
                $body = \str_replace(\var_export($originalClass->name, \true), '__CLASS__', $body);
            }
            $method->setBody($body);
        }
    ...
    public function getProxifiedClass(\Builderius\Symfony\Component\DependencyInjection\Definition $definition) : ?string
    {
