...
use Builderius\Symfony\Component\DependencyInjection\Exception\InvalidParameterTypeException;
 ...
 */
final class CheckTypeDeclarationsPass extends \Builderius\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    ...
    protected function processValue($value, $isRoot = \false)
    {
        ...
        if (!$value instanceof \Builderius\Symfony\Component\DependencyInjection\Definition || $value->hasErrors()) {
            return parent::processValue($value, $isRoot);
        }
        if (!$this->autoload && !\class_exists($class = $value->getClass(), \false) && !\interface_exists($class, \false)) {
            return parent::processValue($value, $isRoot);
        }
        if (\Builderius\Symfony\Component\DependencyInjection\ServiceLocator::class === $value->getClass()) {
            return parent::processValue($value, $isRoot);
        }
     ...
     */
    private function checkTypeDeclarations(\Builderius\Symfony\Component\DependencyInjection\Definition $checkedDefinition, \ReflectionFunctionAbstract $reflectionFunction, array $values) : void
    {
        $numberOfRequiredParameters = $reflectionFunction->getNumberOfRequiredParameters();
        if (\count($values) < $numberOfRequiredParameters) {
            throw new \Builderius\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Invalid definition for service "%s": "%s::%s()" requires %d arguments, %d passed.', $this->currentId, $reflectionFunction->class, $reflectionFunction->name, $numberOfRequiredParameters, \count($values)));
        }
        $reflectionParameters = $reflectionFunction->getParameters();
        $checksCount = \min($reflectionFunction->getNumberOfParameters(), \count($values));
        $envPlaceholderUniquePrefix = $this->container->getParameterBag() instanceof \Builderius\Symfony\Component\DependencyInjection\ParameterBag\EnvPlaceholderParameterBag ? $this->container->getParameterBag()->getEnvPlaceholderUniquePrefix() : null;
     ...
     */
    private function checkType(\Builderius\Symfony\Component\DependencyInjection\Definition $checkedDefinition, $value, \ReflectionParameter $parameter, ?string $envPlaceholderUniquePrefix, string $type = null) : void
    {
        if (null === $type) {
            ...
            if ($type instanceof \ReflectionUnionType) {
                foreach ($type->getTypes() as $type) {
                    try {
                        ...
                        return;
                    } catch (\Builderius\Symfony\Component\DependencyInjection\Exception\InvalidParameterTypeException $e) {
                    }
                }
                throw new \Builderius\Symfony\Component\DependencyInjection\Exception\InvalidParameterTypeException($this->currentId, $e->getCode(), $parameter);
            }
            $type = $type->getName();
        }
        if ($value instanceof \Builderius\Symfony\Component\DependencyInjection\Reference) {
            ...
            if ('service_container' === $value && \is_a($type, \Builderius\Symfony\Component\DependencyInjection\Container::class, \true)) {
                return;
            }
        ...
        if (null === $class) {
            if ($value instanceof \Builderius\Symfony\Component\DependencyInjection\Argument\IteratorArgument) {
                $class = \Builderius\Symfony\Component\DependencyInjection\Argument\RewindableGenerator::class;
                ...
                $class = \Builderius\Symfony\Component\DependencyInjection\ServiceLocator::class;
        ...
        if ('callable' === $type && \is_array($value) && isset($value[0]) && ($value[0] instanceof \Builderius\Symfony\Component\DependencyInjection\Reference || $value[0] instanceof \Builderius\Symfony\Component\DependencyInjection\Definition || \is_string($value[0]))) {
            return;
        }
        ...
        if (!$parameter->getType()->isBuiltin() || !$checkFunction($value)) {
            throw new \Builderius\Symfony\Component\DependencyInjection\Exception\InvalidParameterTypeException($this->currentId, \is_object($value) ? $class : \gettype($value), $parameter);
        }
    }
    private function getExpressionLanguage() : \Builderius\Symfony\Component\DependencyInjection\ExpressionLanguage
    {
        if (null === $this->expressionLanguage) {
            $this->expressionLanguage = new \Builderius\Symfony\Component\DependencyInjection\ExpressionLanguage(null, $this->container->getExpressionLanguageProviders());
        }
