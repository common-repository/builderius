...
use Builderius\Symfony\Component\Console\Formatter\OutputFormatter;
...
use Builderius\Symfony\Component\HttpKernel\Debug\FileLinkFormatter;
 ...
 */
class DebugCommand extends \Builderius\Symfony\Component\Console\Command\Command
{
    ...
    public function __construct(\Builderius\Twig\Environment $twig, string $projectDir = null, array $bundlesMetadata = [], string $twigDefaultPath = null, \Builderius\Symfony\Component\HttpKernel\Debug\FileLinkFormatter $fileLinkFormatter = null)
    {
        parent::__construct();
        $this->twig = $twig;
    ...
    {
        $this->setDefinition([new \Builderius\Symfony\Component\Console\Input\InputArgument('name', \Builderius\Symfony\Component\Console\Input\InputArgument::OPTIONAL, 'The template name'), new \Builderius\Symfony\Component\Console\Input\InputOption('filter', null, \Builderius\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'Show details for all entries matching this filter'), new \Builderius\Symfony\Component\Console\Input\InputOption('format', null, \Builderius\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'The output format (text or json)', 'text')])->setDescription('Shows a list of twig functions, filters, globals and tests')->setHelp(<<<'EOF'
...
EOF
    ...
    protected function execute(\Builderius\Symfony\Component\Console\Input\InputInterface $input, \Builderius\Symfony\Component\Console\Output\OutputInterface $output)
    {
        ...
        if (null !== $name && [] === $this->getFilesystemLoaders()) {
            throw new \Builderius\Symfony\Component\Console\Exception\InvalidArgumentException(\sprintf('Argument "name" not supported, it requires the Twig loader "%s".', \Builderius\Twig\Loader\FilesystemLoader::class));
        }
    ...
    private function displayPathsText(\Builderius\Symfony\Component\Console\Style\SymfonyStyle $io, string $name)
    {
        ...
        if ($file->valid()) {
            ...
            if ($paths) {
                ...
                foreach (\Builderius\Symfony\Component\Finder\Finder::create()->files()->followLinks()->in($dirs) as $file) {
                ...
                if (\Builderius\Twig\Loader\FilesystemLoader::MAIN_NAMESPACE !== $namespace) {
    ...
    private function displayPathsJson(\Builderius\Symfony\Component\Console\Style\SymfonyStyle $io, string $name)
    {
    ...
    private function displayGeneralText(\Builderius\Symfony\Component\Console\Style\SymfonyStyle $io, string $filter = null)
    {
    ...
    private function displayGeneralJson(\Builderius\Symfony\Component\Console\Style\SymfonyStyle $io, ?string $filter)
    {
        $decorated = $io->isDecorated();
        $types = ['functions', 'filters', 'tests', 'globals'];
        $data = [];
        ...
        $io->writeln($decorated ? \Builderius\Symfony\Component\Console\Formatter\OutputFormatter::escape($data) : $data);
    }
    ...
    private function getPrettyMetadata(string $type, $entity, bool $decorated) : ?string
    {
        ...
        try {
            ...
            return \sprintf(' <error>%s</error>', $decorated ? \Builderius\Symfony\Component\Console\Formatter\OutputFormatter::escape($e->getMessage()) : $e->getMessage());
        }
        if ('globals' === $type) {
            if (\is_object($meta)) {
            ...
            return \sprintf(' = %s', $decorated ? \Builderius\Symfony\Component\Console\Formatter\OutputFormatter::escape($description) : $description);
        }
        if ('functions' === $type) {
            return '(' . \implode(', ', $meta) . ')';
        }
    ...
    private function error(\Builderius\Symfony\Component\Console\Style\SymfonyStyle $io, string $message, array $alternatives = []) : void
    {
        if ($alternatives) {
            if (1 === \count($alternatives)) {
                $message .= "\n\nDid you mean this?\n    ";
    ...
    private function parseTemplateName(string $name, string $default = \Builderius\Twig\Loader\FilesystemLoader::MAIN_NAMESPACE) : array
    {
        if (isset($name[0]) && '@' === $name[0]) {
            if (\false === ($pos = \strpos($name, '/')) || $pos === \strlen($name) - 1) {
                throw new \Builderius\Symfony\Component\Console\Exception\InvalidArgumentException(\sprintf('Malformed namespaced template name "%s" (expecting "@namespace/template_name").', $name));
            }
    ...
    private function getFilesystemLoaders() : array
    {
        ...
        } elseif ($loader instanceof \Builderius\Twig\Loader\ChainLoader) {
            foreach ($loader->getLoaders() as $l) {
                if ($l instanceof \Builderius\Twig\Loader\FilesystemLoader) {
