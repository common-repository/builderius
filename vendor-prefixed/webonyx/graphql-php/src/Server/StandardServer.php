...
class StandardServer
{
     ...
     * @api
     */
    public static function send500Error($error, $debug = \Builderius\GraphQL\Error\DebugFlag::NONE, $exitWhenDone = \false)
    {
        $response = ['errors' => [\Builderius\GraphQL\Error\FormattedError::createFromException($error, $debug)]];
        $helper = new \Builderius\GraphQL\Server\Helper();
    ...
    {
        if (\is_array($config)) {
            $config = \Builderius\GraphQL\Server\ServerConfig::create($config);
        }
        if (!$config instanceof \Builderius\GraphQL\Server\ServerConfig) {
            throw new \Builderius\GraphQL\Error\InvariantViolation('Expecting valid server config, but got ' . \Builderius\GraphQL\Utils\Utils::printSafe($config));
        }
        $this->config = $config;
        $this->helper = new \Builderius\GraphQL\Server\Helper();
    }
    /**
     ...
     * @api
     */
    public function processPsrRequest(\Builderius\Psr\Http\Message\RequestInterface $request, \Builderius\Psr\Http\Message\ResponseInterface $response, \Builderius\Psr\Http\Message\StreamInterface $writableBodyStream)
    {
     ...
     * @api
     */
    public function executePsrRequest(\Builderius\Psr\Http\Message\RequestInterface $request)
    {
