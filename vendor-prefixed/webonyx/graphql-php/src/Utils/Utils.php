...
class Utils
{
    ...
    public static function assign($obj, array $vars, array $requiredKeys = [])
    {
        ...
        foreach ($vars as $key => $value) {
            if (!\property_exists($obj, $key)) {
                $cls = \get_class($obj);
                \Builderius\GraphQL\Error\Warning::warn(\sprintf("Trying to set non-existing property '%s' on class '%s'", $key, $cls), \Builderius\GraphQL\Error\Warning::WARNING_ASSIGN);
            }
            $obj->{$key} = $value;
        }
        return $obj;
    ...
    {
        self::invariant(\is_array($iterable) || $iterable instanceof \Traversable, __METHOD__ . ' expects array or Traversable');
    ...
    public static function invariant($test, $message = '')
    {
        if (!$test) {
            ...
            throw new \Builderius\GraphQL\Error\InvariantViolation($message);
        }
    }
    /**
    ...
    public static function printCharCode($code)
    {
        ...
        return $code < 0x7f ? \json_encode(self::chr($code)) : '"\\u' . \dechex($code) . '"';
    }
    /**
    ...
    public static function isValidNameError($name, $node = null)
    {
        ...
        if (isset($name[1]) && $name[0] === '_' && $name[1] === '_') {
            return new \Builderius\GraphQL\Error\Error(\sprintf('Name "%s" must not begin with "__", which is reserved by ', $name) . 'GraphQL introspection.', $node);
        }
        if (!\preg_match('/^[_a-zA-Z][_a-zA-Z0-9]*$/', $name)) {
            return new \Builderius\GraphQL\Error\Error(\sprintf('Names must match /^[_a-zA-Z][_a-zA-Z0-9]*$/ but "%s" does not.', $name), $node);
        }
        return null;
    }
    /**
    ...
    public static function orList(array $items)
    {
        ...
        return \array_reduce(\range(1, $selectedLength - 1), static function ($list, $index) use($selected, $selectedLength) : string {
            return $list . ($selectedLength > 2 ? ', ' : ' ') . ($index === $selectedLength - 1 ? 'or ' : '') . $selected[$index];
        }, $firstSelected);
    }
    /**
    ...
    public static function suggestionList($input, array $options)
    {
        ...
        foreach ($options as $option) {
            if ($input === $option) {
                ...
                $distance = \strtolower($input) === \strtolower($option) ? 1 : \levenshtein($input, $option);
            }
