...
class Value
{
     ...
     * @param mixed[]                                                $path
     */
    public static function coerceValue($value, \Builderius\GraphQL\Type\Definition\InputType $type, $blameNode = null, ?array $path = null)
    {
        if ($type instanceof \Builderius\GraphQL\Type\Definition\NonNull) {
            if ($value === null) {
                return self::ofErrors([self::coercionError(\sprintf('Expected non-nullable type %s not to be null', $type), $blameNode, $path)]);
            }
        ...
        if ($type instanceof \Builderius\GraphQL\Type\Definition\ScalarType) {
            ...
            try {
                ...
                return self::ofErrors([self::coercionError(\sprintf('Expected type %s', $type->name), $blameNode, $path, $error->getMessage(), $error)]);
            }
        }
        if ($type instanceof \Builderius\GraphQL\Type\Definition\EnumType) {
            ...
            $suggestions = \Builderius\GraphQL\Utils\Utils::suggestionList(\Builderius\GraphQL\Utils\Utils::printSafe($value), \array_map(static function ($enumValue) : string {
                return $enumValue->name;
            }, $type->getValues()));
            $didYouMean = $suggestions ? 'did you mean ' . \Builderius\GraphQL\Utils\Utils::orList($suggestions) . '?' : null;
            return self::ofErrors([self::coercionError(\sprintf('Expected type %s', $type->name), $blameNode, $path, $didYouMean)]);
        }
        if ($type instanceof \Builderius\GraphQL\Type\Definition\ListOfType) {
            ...
            if (\is_array($value) || $value instanceof \Traversable) {
                ...
                foreach ($value as $index => $itemValue) {
                    $coercedItem = self::coerceValue($itemValue, $itemType, $blameNode, self::atPath($path, $index));
        ...
        if ($type instanceof \Builderius\GraphQL\Type\Definition\InputObjectType) {
            if (!\is_object($value) && !\is_array($value) && !$value instanceof \Traversable) {
                return self::ofErrors([self::coercionError(\sprintf('Expected type %s to be an object', $type->name), $blameNode, $path)]);
            }
            ...
            foreach ($fields as $fieldName => $field) {
                if (\array_key_exists($fieldName, $value)) {
                    ...
                    $coercedField = self::coerceValue($fieldValue, $field->getType(), $blameNode, self::atPath($path, $fieldName));
                    ...
                    $errors = self::add($errors, self::coercionError(\sprintf('Field %s of required type %s was not provided', $fieldPath, $field->getType()->toString()), $blameNode));
                }
            }
            ...
            foreach ($value as $fieldName => $field) {
                ...
                $suggestions = \Builderius\GraphQL\Utils\Utils::suggestionList((string) $fieldName, \array_keys($fields));
                $didYouMean = $suggestions ? 'did you mean ' . \Builderius\GraphQL\Utils\Utils::orList($suggestions) . '?' : null;
                $errors = self::add($errors, self::coercionError(\sprintf('Field "%s" is not defined by type %s', $fieldName, $type->name), $blameNode, $path, $didYouMean));
            }
            return $errors ? self::ofErrors($errors) : self::ofValue($coercedValue);
        }
        throw new \Builderius\GraphQL\Error\Error(\sprintf('Unexpected type %s', $type->name));
    }
    private static function ofErrors($errors)
    {
        return ['errors' => $errors, 'value' => \Builderius\GraphQL\Utils\Utils::undefined()];
    }
    /**
    ...
    private static function coercionError($message, $blameNode, ?array $path = null, $subMessage = null, $originalError = null)
    {
        ...
        return new \Builderius\GraphQL\Error\Error($message . ($pathStr ? ' at ' . $pathStr : '') . ($subMessage ? '; ' . $subMessage : '.'), $blameNode, null, [], null, $originalError);
    }
    /**
    ...
    private static function printPath(?array $path = null)
    {
        ...
        $currentPath = $path;
        while ($currentPath) {
            $pathStr = (\is_string($currentPath['key']) ? '.' . $currentPath['key'] : '[' . $currentPath['key'] . ']') . $pathStr;
