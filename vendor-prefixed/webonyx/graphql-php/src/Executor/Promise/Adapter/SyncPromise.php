...
class SyncPromise
{
        ...
        switch ($this->state) {
            case self::PENDING:
                ...
                if (\is_object($value) && \method_exists($value, 'then')) {
                    $value->then(function ($resolvedValue) : void {
    ...
    {
        \Builderius\GraphQL\Utils\Utils::invariant($this->state !== self::PENDING, 'Cannot enqueue derived promises when parent is still pending');
