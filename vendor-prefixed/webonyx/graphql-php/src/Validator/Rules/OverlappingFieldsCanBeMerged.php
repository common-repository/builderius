...
use Builderius\GraphQL\Utils\PairSet;
...
class OverlappingFieldsCanBeMerged extends \Builderius\GraphQL\Validator\Rules\ValidationRule
{
    /**
    ...
    {
        $this->comparedFragmentPairs = new \Builderius\GraphQL\Utils\PairSet();
        $this->cachedFieldsAndFragmentNames = new \SplObjectStorage();
        return [\Builderius\GraphQL\Language\AST\NodeKind::SELECTION_SET => function (\Builderius\GraphQL\Language\AST\SelectionSetNode $selectionSet) use($context) : void {
            $conflicts = $this->findConflictsWithinSelectionSet($context, $context->getParentType(), $selectionSet);
            foreach ($conflicts as $conflict) {
                [[$responseName, $reason], $fields1, $fields2] = $conflict;
                $context->reportError(new \Builderius\GraphQL\Error\Error(self::fieldsConflictMessage($responseName, $reason), \array_merge($fields1, $fields2)));
            }
        }];
    }
    /**
     ...
     * @return mixed[]
     */
    private function findConflictsWithinSelectionSet(\Builderius\GraphQL\Validator\ValidationContext $context, $parentType, \Builderius\GraphQL\Language\AST\SelectionSetNode $selectionSet)
    {
        [$fieldMap, $fragmentNames] = $this->getFieldsAndFragmentNames($context, $parentType, $selectionSet);
        $conflicts = [];
     ...
     */
    private function getFieldsAndFragmentNames(\Builderius\GraphQL\Validator\ValidationContext $context, $parentType, \Builderius\GraphQL\Language\AST\SelectionSetNode $selectionSet)
    {
     ...
     */
    private function internalCollectFieldsAndFragmentNames(\Builderius\GraphQL\Validator\ValidationContext $context, $parentType, \Builderius\GraphQL\Language\AST\SelectionSetNode $selectionSet, array &$astAndDefs, array &$fragmentNames)
    {
        foreach ($selectionSet->selections as $selection) {
            switch (\true) {
                ...
                case $selection instanceof \Builderius\GraphQL\Language\AST\FragmentSpreadNode:
                    $fragmentNames[$selection->name->value] = \true;
                    break;
                case $selection instanceof \Builderius\GraphQL\Language\AST\InlineFragmentNode:
                    $typeCondition = $selection->typeCondition;
                    $inlineFragmentType = $typeCondition ? \Builderius\GraphQL\Utils\TypeInfo::typeFromAST($context->getSchema(), $typeCondition) : $parentType;
     ...
     */
    private function collectConflictsWithin(\Builderius\GraphQL\Validator\ValidationContext $context, array &$conflicts, array $fieldMap)
    {
     ...
     * @return mixed[]|null
     */
    private function findConflict(\Builderius\GraphQL\Validator\ValidationContext $context, $parentFieldsAreMutuallyExclusive, $responseName, array $field1, array $field2)
    {
        ...
        $areMutuallyExclusive = $parentFieldsAreMutuallyExclusive || $parentType1 !== $parentType2 && $parentType1 instanceof \Builderius\GraphQL\Type\Definition\ObjectType && $parentType2 instanceof \Builderius\GraphQL\Type\Definition\ObjectType;
        ...
        if (!$areMutuallyExclusive) {
            ...
            if ($name1 !== $name2) {
                return [[$responseName, \sprintf('%s and %s are different fields', $name1, $name2)], [$ast1], [$ast2]];
            }
        ...
        if ($type1 && $type2 && $this->doTypesConflict($type1, $type2)) {
            return [[$responseName, \sprintf('they return conflicting types %s and %s', $type1, $type2)], [$ast1], [$ast2]];
        }
        ...
        if ($selectionSet1 && $selectionSet2) {
            $conflicts = $this->findConflictsBetweenSubSelectionSets($context, $areMutuallyExclusive, \Builderius\GraphQL\Type\Definition\Type::getNamedType($type1), $selectionSet1, \Builderius\GraphQL\Type\Definition\Type::getNamedType($type2), $selectionSet2);
     ...
     * @return bool
     */
    private function sameValue(\Builderius\GraphQL\Language\AST\Node $value1, \Builderius\GraphQL\Language\AST\Node $value2)
    {
        return !$value1 && !$value2 || \Builderius\GraphQL\Language\Printer::doPrint($value1) === \Builderius\GraphQL\Language\Printer::doPrint($value2);
    }
    /**
     ...
     */
    private function doTypesConflict(\Builderius\GraphQL\Type\Definition\Type $type1, \Builderius\GraphQL\Type\Definition\Type $type2) : bool
    {
        if ($type1 instanceof \Builderius\GraphQL\Type\Definition\ListOfType) {
            return $type2 instanceof \Builderius\GraphQL\Type\Definition\ListOfType ? $this->doTypesConflict($type1->getWrappedType(), $type2->getWrappedType()) : \true;
        }
        if ($type2 instanceof \Builderius\GraphQL\Type\Definition\ListOfType) {
            return $type1 instanceof \Builderius\GraphQL\Type\Definition\ListOfType ? $this->doTypesConflict($type1->getWrappedType(), $type2->getWrappedType()) : \true;
        }
        if ($type1 instanceof \Builderius\GraphQL\Type\Definition\NonNull) {
            return $type2 instanceof \Builderius\GraphQL\Type\Definition\NonNull ? $this->doTypesConflict($type1->getWrappedType(), $type2->getWrappedType()) : \true;
        }
        if ($type2 instanceof \Builderius\GraphQL\Type\Definition\NonNull) {
            return $type1 instanceof \Builderius\GraphQL\Type\Definition\NonNull ? $this->doTypesConflict($type1->getWrappedType(), $type2->getWrappedType()) : \true;
        }
        if (\Builderius\GraphQL\Type\Definition\Type::isLeafType($type1) || \Builderius\GraphQL\Type\Definition\Type::isLeafType($type2)) {
            return $type1 !== $type2;
        }
        return \false;
    }
     ...
     * @return mixed[][]
     */
    private function findConflictsBetweenSubSelectionSets(\Builderius\GraphQL\Validator\ValidationContext $context, $areMutuallyExclusive, $parentType1, \Builderius\GraphQL\Language\AST\SelectionSetNode $selectionSet1, $parentType2, \Builderius\GraphQL\Language\AST\SelectionSetNode $selectionSet2)
    {
        $conflicts = [];
        [$fieldMap1, $fragmentNames1] = $this->getFieldsAndFragmentNames($context, $parentType1, $selectionSet1);
        [$fieldMap2, $fragmentNames2] = $this->getFieldsAndFragmentNames($context, $parentType2, $selectionSet2);
     ...
     */
    private function collectConflictsBetween(\Builderius\GraphQL\Validator\ValidationContext $context, array &$conflicts, $parentFieldsAreMutuallyExclusive, array $fieldMap1, array $fieldMap2)
    {
        // A field map is a keyed collection, where each key represents a response
        // name and the value at that key is a list of all fields which provide that
        ...
        foreach ($fieldMap1 as $responseName => $fields1) {
            ...
            for ($i = 0; $i < $fields1Length; $i++) {
                for ($j = 0; $j < $fields2Length; $j++) {
                    $conflict = $this->findConflict($context, $parentFieldsAreMutuallyExclusive, $responseName, $fields1[$i], $fields2[$j]);
                    if (!$conflict) {
                        continue;
                    }
                    $conflicts[] = $conflict;
     ...
     */
    private function collectConflictsBetweenFieldsAndFragment(\Builderius\GraphQL\Validator\ValidationContext $context, array &$conflicts, array &$comparedFragments, $areMutuallyExclusive, array $fieldMap, $fragmentName)
    {
        if (isset($comparedFragments[$fragmentName])) {
            return;
        }
        $comparedFragments[$fragmentName] = \true;
        $fragment = $context->getFragment($fragmentName);
        if (!$fragment) {
            return;
        }
        [$fieldMap2, $fragmentNames2] = $this->getReferencedFieldsAndFragmentNames($context, $fragment);
     ...
     * @return mixed[]|SplObjectStorage
     */
    private function getReferencedFieldsAndFragmentNames(\Builderius\GraphQL\Validator\ValidationContext $context, \Builderius\GraphQL\Language\AST\FragmentDefinitionNode $fragment)
    {
        ...
        $fragmentType = \Builderius\GraphQL\Utils\TypeInfo::typeFromAST($context->getSchema(), $fragment->typeCondition);
     ...
     */
    private function collectConflictsBetweenFragments(\Builderius\GraphQL\Validator\ValidationContext $context, array &$conflicts, $areMutuallyExclusive, $fragmentName1, $fragmentName2)
    {
        ...
        [$fieldMap1, $fragmentNames1] = $this->getReferencedFieldsAndFragmentNames($context, $fragment1);
        [$fieldMap2, $fragmentNames2] = $this->getReferencedFieldsAndFragmentNames($context, $fragment2);
     ...
     * @return mixed[]|null
     */
    private function subfieldConflicts(array $conflicts, $responseName, \Builderius\GraphQL\Language\AST\FieldNode $ast1, \Builderius\GraphQL\Language\AST\FieldNode $ast2)
    {
        if (\count($conflicts) === 0) {
            return null;
        }
        return [[$responseName, \array_map(static function ($conflict) {
            return $conflict[0];
        }, $conflicts)], \array_reduce($conflicts, static function ($allFields, $conflict) : array {
            return \array_merge($allFields, $conflict[1]);
        }, [$ast1]), \array_reduce($conflicts, static function ($allFields, $conflict) : array {
    ...
    public static function fieldsConflictMessage($responseName, $reason)
    {
        ...
        return \sprintf('Fields "%s" conflict because %s. Use different aliases on the fields to fetch both if this was intentional.', $responseName, $reasonMessage);
    }
    public static function reasonMessage($reason)
    {
        if (\is_array($reason)) {
            $tmp = \array_map(static function ($tmp) : string {
