...
namespace Builderius\GraphQL\Type\Validation;
...
use Builderius\GraphQL\Type\SchemaValidationContext;
...
class InputObjectCircularRefs
{
    ...
    public function __construct(\Builderius\GraphQL\Type\SchemaValidationContext $schemaValidationContext)
    {
     ...
     */
    public function validate(\Builderius\GraphQL\Type\Definition\InputObjectType $inputObj) : void
    {
        if (isset($this->visitedTypes[$inputObj->name])) {
            return;
        }
        $this->visitedTypes[$inputObj->name] = \true;
        ...
        foreach ($fieldMap as $fieldName => $field) {
            ...
            if ($type instanceof \Builderius\GraphQL\Type\Definition\NonNull) {
                ...
                if ($fieldType instanceof \Builderius\GraphQL\Type\Definition\InputObjectType) {
                    ...
                    if (!isset($this->fieldPathIndexByTypeName[$fieldType->name])) {
                        ...
                        $fieldNames = \array_map(static function (\Builderius\GraphQL\Type\Definition\InputObjectField $field) : string {
                            return $field->name;
                        }, $cyclePath);
                        $this->schemaValidationContext->reportError('Cannot reference Input Object "' . $fieldType->name . '" within itself ' . 'through a series of non-null fields: "' . \implode('.', $fieldNames) . '".', \array_map(static function (\Builderius\GraphQL\Type\Definition\InputObjectField $field) : ?InputValueDefinitionNode {
