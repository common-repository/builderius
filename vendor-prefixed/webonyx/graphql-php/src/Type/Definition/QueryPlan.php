...
class QueryPlan
{
     ...
     * @param mixed[]                  $options
     */
    public function __construct(\Builderius\GraphQL\Type\Definition\ObjectType $parentType, \Builderius\GraphQL\Type\Schema $schema, iterable $fieldNodes, array $variableValues, array $fragments, array $options = [])
    {
        $this->schema = $schema;
        $this->variableValues = $variableValues;
        $this->fragments = $fragments;
        $this->groupImplementorFields = \in_array('group-implementor-fields', $options, \true);
     ...
     * @param FieldNode[] $fieldNodes
     */
    private function analyzeQueryPlan(\Builderius\GraphQL\Type\Definition\ObjectType $parentType, iterable $fieldNodes) : void
    {
        $queryPlan = [];
        $implementors = [];
        ...
        foreach ($fieldNodes as $fieldNode) {
            ...
            $this->types[$type->name] = \array_unique(\array_merge(\array_key_exists($type->name, $this->types) ? $this->types[$type->name] : [], \array_keys($subfields)));
            $queryPlan = \array_merge_recursive($queryPlan, $subfields);
        }
     ...
     * @throws Error
     */
    private function analyzeSelectionSet(\Builderius\GraphQL\Language\AST\SelectionSetNode $selectionSet, \Builderius\GraphQL\Type\Definition\Type $parentType, array &$implementors) : array
    {
        $fields = [];
        $implementors = [];
        foreach ($selectionSet->selections as $selectionNode) {
            if ($selectionNode instanceof \Builderius\GraphQL\Language\AST\FieldNode) {
                ...
                $fields[$fieldName] = ['type' => $selectionType, 'fields' => $subfields ?? [], 'args' => \Builderius\GraphQL\Executor\Values::getArgumentValues($type, $selectionNode, $this->variableValues)];
                if ($this->groupImplementorFields && $subImplementors) {
                    $fields[$fieldName]['implementors'] = $subImplementors;
                }
            } elseif ($selectionNode instanceof \Builderius\GraphQL\Language\AST\FragmentSpreadNode) {
            ...
            } elseif ($selectionNode instanceof \Builderius\GraphQL\Language\AST\InlineFragmentNode) {
     ...
     * @return mixed[]
     */
    private function analyzeSubFields(\Builderius\GraphQL\Type\Definition\Type $type, \Builderius\GraphQL\Language\AST\SelectionSetNode $selectionSet, array &$implementors = []) : array
    {
        if ($type instanceof \Builderius\GraphQL\Type\Definition\WrappingType) {
            $type = $type->getWrappedType();
        }
        $subfields = [];
        if ($type instanceof \Builderius\GraphQL\Type\Definition\ObjectType || $type instanceof \Builderius\GraphQL\Type\Definition\AbstractType) {
     ...
     * @return mixed[]
     */
    private function mergeFields(\Builderius\GraphQL\Type\Definition\Type $parentType, \Builderius\GraphQL\Type\Definition\Type $type, array $fields, array $subfields, array &$implementors) : array
    {
        if ($this->groupImplementorFields && $parentType instanceof \Builderius\GraphQL\Type\Definition\AbstractType && !$type instanceof \Builderius\GraphQL\Type\Definition\AbstractType) {
            $implementors[$type->name] = ['type' => $type, 'fields' => $this->arrayMergeDeep($implementors[$type->name]['fields'] ?? [], \array_diff_key($subfields, $fields))];
            $fields = $this->arrayMergeDeep($fields, \array_intersect_key($subfields, $fields));
        } else {
