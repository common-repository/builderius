...
class ResolveInfo
{
     ...
     */
    public function __construct(\Builderius\GraphQL\Type\Definition\FieldDefinition $fieldDefinition, iterable $fieldNodes, \Builderius\GraphQL\Type\Definition\ObjectType $parentType, array $path, \Builderius\GraphQL\Type\Schema $schema, array $fragments, $rootValue, ?\Builderius\GraphQL\Language\AST\OperationDefinitionNode $operation, array $variableValues)
    {
     ...
     * @param mixed[] $options
     */
    public function lookAhead(array $options = []) : \Builderius\GraphQL\Type\Definition\QueryPlan
    {
        if (!isset($this->queryPlan)) {
            $this->queryPlan = new \Builderius\GraphQL\Type\Definition\QueryPlan($this->parentType, $this->schema, $this->fieldNodes, $this->variableValues, $this->fragments, $options);
        }
     ...
     * @return bool[]
     */
    private function foldSelectionSet(\Builderius\GraphQL\Language\AST\SelectionSetNode $selectionSet, int $descend) : array
    {
        $fields = [];
        foreach ($selectionSet->selections as $selectionNode) {
            if ($selectionNode instanceof \Builderius\GraphQL\Language\AST\FieldNode) {
                $fields[$selectionNode->name->value] = $descend > 0 && $selectionNode->selectionSet !== null ? $this->foldSelectionSet($selectionNode->selectionSet, $descend - 1) : \true;
