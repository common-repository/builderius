...
class NodeList implements \ArrayAccess, \IteratorAggregate, \Countable
{
    ...
    public function offsetGet($offset)
    {
        ...
        if (\is_array($item) && isset($item['kind'])) {
            /** @phpstan-var T $node */
            $node = \Builderius\GraphQL\Utils\AST::fromArray($item);
    ...
    public function offsetSet($offset, $value) : void
    {
        if (\is_array($value)) {
            if (isset($value['kind'])) {
                /** @phpstan-var T $node */
                $node = \Builderius\GraphQL\Utils\AST::fromArray($value);
                $this->nodes[$offset] = $node;
                return;
            }
            throw new \InvalidArgumentException('Expected array value to be valid node data structure, missing key "kind"');
        }
     ...
     */
    public function splice(int $offset, int $length, $replacement = null) : \Builderius\GraphQL\Language\AST\NodeList
    {
        return new \Builderius\GraphQL\Language\AST\NodeList(\array_splice($this->nodes, $offset, $length, $replacement));
    }
    /**
     ...
     * @phpstan-return NodeList<T>
     */
    public function merge($list) : \Builderius\GraphQL\Language\AST\NodeList
    {
        if ($list instanceof self) {
            $list = $list->nodes;
        }
        return new \Builderius\GraphQL\Language\AST\NodeList(\array_merge($this->nodes, $list));
    }
